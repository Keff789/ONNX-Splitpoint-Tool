...
from onnx_splitpoint_tool import v279_smoke, v27922_smoke
from onnx_splitpoint_tool.release_identity import BUILD_ID, VERSION
from onnx_splitpoint_tool.workflow.runner import WORKFLOW_VERSION


ROOT = Path(__file__).resolve().parents[1]
EXPECTED_VERSION = "2.79.22"
EXPECTED_BUILD = "v2.79.22-global-negative-build-evidence"
...
def test_current_identity_and_smoke_alias() -> None:
    ...
    assert v27922_smoke.VERSION == v279_smoke.VERSION == EXPECTED_VERSION
    assert v27922_smoke.BUILD_ID == v279_smoke.BUILD_ID == EXPECTED_BUILD


def test_current_entrypoints_retain_historical_aliases() -> None:
    pyproject = _read("pyproject.toml")
    assert 'version = "2.79.22"' in pyproject
    assert 'name = "onnx-splitpoint-tool"\nversion = "2.79.22"' in _read("uv.lock")
    ...
    for path in ("scripts/run_v279_small_acceptance.sh", "scripts/run_local_acceptance.sh"):
        assert "run_v27922_small_acceptance.sh" in _read(path)
...
def test_updater_exact_identity_and_retained_entrypoints() -> None:
    ...
    for marker in (
        ...
        "ONNX-Splitpoint-Tool_v2.79.22",
        "--expected-version 2.79.22",
        "--run-entrypoint onnx-splitpoint-smoke-v27922",
    ):
...
def test_release_docs_are_current_and_historical_docs_are_retained() -> None:
    for path in ("TESTANLEITUNG_2.79.22.md", "VERSION_2.79.22_BUILD_AND_TEST_REPORT.md"):
        contents = _read(path)
...
def test_current_gate_retains_behavioral_regressions_not_old_identity_gate() -> None:
    gate = _read("scripts/run_v27922_small_acceptance.sh")
    assert "tests/test_v27922_release_closure.py" in gate
    for test in (
...
def test_current_hardware_independent_smoke(capsys) -> None:
    assert v27922_smoke.main() == 0
    assert "PASS v2.79.22 smoke" in capsys.readouterr().out
