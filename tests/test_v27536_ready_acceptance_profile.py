...
PROFILE = ROOT / "profiles" / "resnet50_v27536_small_acceptance.yaml"


def test_packaged_resnet50_v27536_profile_has_exact_small_scope() -> None:
    source = yaml.safe_load(PROFILE.read_text(encoding="utf-8"))

    assert source["name"] == "resnet50_v27536_small_acceptance"
...
def test_packaged_resnet50_v27536_profile_resolves_energy_and_probe_off(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setenv(
