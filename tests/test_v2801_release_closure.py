...
from onnx_splitpoint_tool import v279_smoke, v2801_smoke
...
EXPECTED_BUILD = "v2.80.1-cpu-reference-remote-closure-debugexport"
...
def test_current_identity_and_smoke_alias() -> None:
    ...
    assert v2801_smoke.VERSION == v279_smoke.VERSION == EXPECTED_VERSION
    assert v2801_smoke.BUILD_ID == v279_smoke.BUILD_ID == EXPECTED_BUILD


def test_current_entrypoints_retain_historical_aliases() -> None:
    pyproject = _read("pyproject.toml")
    assert 'version = "2.80.1"' in pyproject
    assert 'name = "onnx-splitpoint-tool"\nversion = "2.80.1"' in _read("uv.lock")
    ...
    for alias in ("v2801", "v2-80-1"):
        assert f'onnx-splitpoint-smoke-{alias} = "onnx_splitpoint_tool.v2801_smoke:main"' in pyproject
    for path in ("scripts/run_v279_small_acceptance.sh", "scripts/run_local_acceptance.sh"):
        assert "run_v2801_small_acceptance.sh" in _read(path)
...
def test_updater_exact_identity_and_retained_entrypoints() -> None:
    ...
    for marker in (
        ...
        "ONNX-Splitpoint-Tool_v2.80.1",
        "--expected-version 2.80.1",
        "--run-entrypoint onnx-splitpoint-smoke-v2801",
    ):
    ...
    for alias in ("v2801", "v2-80-1"):
        assert f"onnx-splitpoint-smoke-{alias}=onnx_splitpoint_tool.v2801_smoke:main" in updater
...
def test_release_docs_are_current_and_historical_docs_are_retained() -> None:
    for path in ("TESTANLEITUNG_2.80.1.md", "VERSION_2.80.1_BUILD_AND_TEST_REPORT.md"):
        contents = _read(path)
...
def test_current_gate_retains_behavioral_regressions_not_old_identity_gate() -> None:
    gate = _read("scripts/run_v2801_small_acceptance.sh")
    assert "tests/test_v2801_release_closure.py" in gate
    for test in (
...
def test_current_hardware_independent_smoke(capsys) -> None:
    assert v2801_smoke.main() == 0
    assert "PASS v2.80.1 smoke" in capsys.readouterr().out
...
assert modules[0].VERSION == "2.80.1"
...
'''
    ...
    assert json.loads(result.stdout.strip().splitlines()[-1])["version"] == "2.80.1"
...
def test_gate_covers_prior_selection_and_all_rev2_regressions() -> None:
    ...
    previous = set(re.findall(r"tests/[^\s\\]+", _read("scripts/run_v280_small_acceptance.sh")))
    current = set(re.findall(r"tests/[^\s\\]+", _read("scripts/run_v2801_small_acceptance.sh")))
    previous.remove("tests/test_v280_release_closure.py")
...
def test_exact_recorded_npz_is_manifested_and_other_binaries_stay_excluded(tmp_path: Path) -> None:
    ...
    updater = _read("scripts/update_source_release.sh")
    embedded = updater[updater.index('EXPECTED_VERSION = "2.80.1"'):]
    predicate = embedded[embedded.index("def valid_logical_path("):embedded.index("def strict_sha256(")]
    ns = {"PurePosixPath": PurePosixPath, "EXPECTED_VERSION": "2.80.1"}
    for name in (
...
def test_required_acceptance_reports_never_pass_skipped_tests(tmp_path: Path, kind: str, expected_rc: int) -> None:
    ...
    required = ('test_v2801_cpu_reference_dispatch', 'test_v2801_native_remote_closure', 'test_v2801_reference_errors', 'test_v2801_reference_debug_export', 'test_v2801_workflow_integration', 'test_v2801_release_scope', 'test_v2801_release_closure')
...
spec = importlib.util.spec_from_file_location("current_full_smoke", root/"scripts/deepx_full_workflow_smoke_v2801.py")
...
'''
    ...
    assert json.loads(result.stdout) == {
        "SOURCE_VERSION": "2.80.1", "SOURCE_TAG": "v2801",
        "SOURCE_NAME": "ONNX-Splitpoint-Tool_v2.80.1_SOURCE.zip",
        "DEFAULT_BUNDLE": "ONNX-Splitpoint-Tool_v2.80.1_COMPLETE_DELIVERY_BUNDLE",
    }
...
def test_T32_R09_current_launchers_parse_in_fresh_isolated_process(name: str, tmp_path: Path) -> None:
    ...
    result = subprocess.run([sys.executable, "-I", "-B", str(ROOT / "scripts" / f"{name}_v2801.py"), "--help"],
                            cwd=tmp_path, text=True, capture_output=True, timeout=45,
                            env={k: v for k, v in os.environ.items() if k not in {"PYTHONPATH", "PYTHONHOME"}})
    assert result.returncode == 0, result.stdout + result.stderr
...
def test_T32_R09_current_gate_preserves_every_previous_case_and_all_new_suites() -> None:
    ...
    previous = set(re.findall(r"tests/[^\s\\]+", _read("scripts/run_v280_small_acceptance.sh")))
    current_text = _read("scripts/run_v2801_small_acceptance.sh")
    current = set(re.findall(r"tests/[^\s\\]+", current_text))
    assert previous - {"tests/test_v280_release_closure.py"} <= current
    ...
    assert "tests/test_v2801_release_closure.py" in current
...
def test_T32_R09_scientific_scope_is_separate_from_software_release() -> None:
    docs = _read("TESTANLEITUNG_2.80.1.md")
...
def _exercise_canonical_workdir_updater(tmp_path: Path, updater_root: Path) -> dict:
    ...
    write_package(target, '2.80', 'v2.80-hailo-reuse-env-cleanup')
    ...
    run([str(py), '-I', '-B', str(target/'scripts/refresh_editable_install.py'), '--root', str(target), '--expected-version', '2.80'])
    payloads = {
