...
from onnx_splitpoint_tool.v270g_smoke import (
    REQUIRED_FEATURES,
    main as smoke_main,
)
...
def test_v270g_identity_package_and_entrypoints_are_consistent() -> None:
    ...
    assert (
        'onnx-splitpoint-smoke-v270g = '
        '"onnx_splitpoint_tool.v270g_smoke:main"'
    ) in pyproject
    assert (
        'onnx-splitpoint-smoke-v2-70g = '
        '"onnx_splitpoint_tool.v270g_smoke:main"'
    ) in pyproject


def test_v270g_release_smoke_and_critical_modules(capsys) -> None:
    assert smoke_main() == 0
    result = json.loads(capsys.readouterr().out)
    assert result["schema"] == "onnx-splitpoint/v270g-smoke"
...
def test_v270g_release_documents_record_current_identity() -> None:
    for path in (
        Path("README.md"),
