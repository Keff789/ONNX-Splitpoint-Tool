"""Fresh v29 worker with cv2 forbidden, real recorded output and real harness."""
...
from scripts import deepx_full_output_probe_worker_v27929 as worker
...
def test_staged_current_worker_real_tensor_and_harness(case, tmp_path, kind):
    original_npz = os.environ.get("V27929_REPLAY_NPZ") or os.environ.get("V27928_REPLAY_NPZ")
    if not original_npz:
        pytest.fail("recorded tensor required: set V27929_REPLAY_NPZ from the delivery bundle")
...
def no_cv2(name, *args, **kwargs):
    if name == "cv2" or name.startswith("cv2."):
        cv2_attempts.append(name)
        raise ModuleNotFoundError("OpenCV forbidden in this regression", name="cv2")
...
try:
    ...
    (Path(request_path).parent / "opencv_import_guard.json").write_text(
        json.dumps({"opencv_import_attempts": cv2_attempts}))
''')
    ...
    assert result['status'] == ('diagnostic_pass' if kind == 'roundoff' else 'runtime_failed'), json.dumps(result, indent=2)
    assert json.loads((work / 'opencv_import_guard.json').read_text())['opencv_import_attempts'] == []
