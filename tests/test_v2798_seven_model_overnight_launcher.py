...
LAUNCHER = ROOT / "scripts/run_v2798_seven_model_long_overnight.sh"
GENERIC_LAUNCHER = ROOT / "scripts/run_v279_seven_model_long_overnight.sh"
STATUS_HELPER = ROOT / "scripts/launcher_status_v2798.py"
...
def test_v2798_launcher_is_source_safe_helpful_and_executable() -> None:
    assert os.access(LAUNCHER, os.X_OK)
    subprocess.run(["bash", "-n", str(LAUNCHER)], check=True)

    help_result = _run("--help")
    assert help_result.returncode == 0
    assert "run_v2798_seven_model_long_overnight.sh" in help_result.stdout
...
def test_r8b_output_option_fails_closed_before_host_preflight() -> None:
    ...
    unsupported = _run("--unsupported-v2798-option")
    assert unsupported.returncode == 64
    assert "unsupported option" in unsupported.stderr


def test_v2798_launcher_pins_release_profile_identity_and_acceptance() -> None:
    text = _read(LAUNCHER)
    assert 'expected = "2.79.8"' in text
    assert "V2798_RELEASE_IDENTITY=PASS" in text
    assert 'ACCEPTANCE="$TOOL/scripts/run_v2798_small_acceptance.sh"' in text
    assert 'STATUS_HELPER="$TOOL/scripts/launcher_status_v2798.py"' in text
    assert 'PROFILE="$TOOL/profiles/complete_set_7models_v2798_b500_audit20.yaml"' in text
    assert (
        "PROFILE_EXPECTED_SHA256="
        "d168d7ab5069a6cd75db43ec59ea5345de02497f6f2a2abb148f9170ff6de5d4"
    ) in text
    assert 'profile.get("name") == "complete_set_7models_v2798_b500_audit20"' in text
...
def test_r8b_receipt_is_exact_and_precedes_every_launch_path() -> None:
    text = _read(LAUNCHER)
    assert 'GATE_VERIFIER="$TOOL/scripts/verify_v2798_yolo11_r8b_gate.py"' in text
    assert 'GATE_PROFILE="$TOOL/profiles/yolo11l_v2798_r8b_full_b067_gate.yaml"' in text
    assert 'RECOVERY_HELPER="$TOOL/scripts/prepare_v2798_yolo11_r8b_recovery.py"' in text
    ...
    assert text.count('"version": "2.79.8"') >= 3
    assert text.count(
        '"build_id": "v2.79.8-yolo11-gate-profile-schema-closure"'
    ) >= 3
    assert text.count(
        '"workflow_version": "v2.79.8-yolo11-gate-profile-schema-closure"'
    ) >= 2
    ...
    assert 'gate_verdict.get("version") == "2.79.8"' in text
    assert 'gate.get("version") != "2.79.8"' in text
    for marker in (
    ...
    gate_pass = text.index('print("V2798_YOLO11_FULL_GATE=PASS")', verify_call)
...
def test_yolov7_claim_gate_is_strict_receipt_bound_and_pre_dispatch() -> None:
    text = _read(LAUNCHER)
    assert 'YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v2798_yolov7_claim_gate_32.py"' in text
    ...
    pass_marker = text.index('print("V2798_YOLOV7_CLAIM_GATE_32=PASS")', verifier)
...
def test_preflight_is_receipt_bound_and_cannot_dispatch() -> None:
    ...
    assert "V2798_SEVEN_MODEL_LONG_PREFLIGHT=PASS" in branch
    assert "V2798_YOLO11_FULL_GATE=PASS" in branch
    assert "V2798_YOLOV7_CLAIM_GATE_32=PASS" in branch
...
def test_v2798_status_helper_is_frozen_and_current_alias_tracks_v27913() -> None:
    ...
    assert "run_v2798_seven_model_long_overnight.sh" not in generic
    ...
    expected = _read(HISTORICAL_STATUS_HELPER).replace(
        "for v2.79.5 campaigns", "for v2.79.8 campaigns"
    )
