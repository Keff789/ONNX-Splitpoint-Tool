...
LAUNCHER = ROOT / "scripts/run_v27910_yolo11_r8b_gate.sh"
...
def test_v27910_gate_launcher_is_source_safe_and_has_valid_bash() -> None:
    ...
    assert "run_v27910_yolo11_r8b_gate.sh" in help_result.stdout
...
def test_standard_venv_python_symlink_passes_semantic_binding(
    ...
    recovery = tmp_path / "v27910_recovery_manifest.json"
    tool.mkdir()
    models.mkdir()
    recovery.write_text("{}\n", encoding="utf-8")
...
def test_launcher_pins_v27910_identity_and_all_prehardware_checks() -> None:
    source = _source()
    assert 'expected_version = "2.79.10"' in source
    assert f'expected_build = "{BUILD_ID}"' in source
    assert 'PROFILE="$TOOL/profiles/yolo11l_v27910_r8b_full_b067_gate.yaml"' in source
    assert 'VERIFIER="$TOOL/scripts/verify_v27910_yolo11_r8b_gate.py"' in source
    assert (
        "PROFILE_SHA256="
        "f1da0895df2185158a09b60c61caa939f129fa5cf81a5dfca50652e6a3ac1294"
        in source
    )
    assert "V27910_R8B_RELEASE_IDENTITY=PASS" in source
    assert "V27910_R8B_SOURCE_MANIFEST=PASS" in source
    assert "V27910_YOLO11_R8B_GATE=PASS" in source
    ...
    assert 'RECOVERY_MANIFEST="$GATE_OUTPUT/v27910_recovery_manifest.json"' in source
    assert "os.O_EXCL" in source
    assert "O_NOFOLLOW" in source

    release = source.index("V27910_R8B_RELEASE_IDENTITY=PASS")
    manifest = source.index("V27910_R8B_SOURCE_MANIFEST=PASS")
...
def test_heredocs_are_not_part_of_an_or_list_and_errors_fail_closed() -> None:
    ...
    assert source.count("\nPY\nthen\n") == 3
    assert 'die 65 "Installierte v2.79.10-Package-/Workflow-Identität stimmt nicht"' in source
