...
REQUIRED_MODULES = ('test_v2802_cpu_reference_binding', 'test_v2802_native_quality_blocking', 'test_v2802_dashboard_projection', 'test_v2802_workflow_integration', 'test_v2802_release_scope', 'test_v2802_release_closure')
...
def _report_python():
    gate = (ROOT/'scripts/run_v2802_small_acceptance.sh').read_text()
...
def test_short_gate_validates_junit_and_preservation(tmp_path, case):
    script = (ROOT/'scripts/run_v2802_short_tests.sh').read_text()
...
def test_current_gates_select_all_new_modules_and_preserve_prior_behavior():
    for current, previous in (
        ('run_v2802_short_tests.sh', 'run_v2801_short_tests.sh'),
        ('run_v2802_small_acceptance.sh', 'run_v2801_small_acceptance.sh'),
    ):
        ...
        previous_selected.discard('tests/test_v2801_release_closure.py')
        assert previous_selected <= selected
        assert {'tests/'+name+'.py' for name in REQUIRED_MODULES} <= selected
        assert 'tests/test_v2801_release_closure.py' not in selected
...
def test_new_required_modules_are_real_selected_files():
    ...
    for gate in ('run_v2802_short_tests.sh', 'run_v2802_small_acceptance.sh'):
...
def test_previous_smoke_feature_contract_is_not_reduced():
    from onnx_splitpoint_tool import v2801_smoke, v2802_smoke, __build_features__
    assert v2801_smoke.REQUIRED_FEATURES < v2802_smoke.REQUIRED_FEATURES
    assert v2802_smoke.REQUIRED_FEATURES <= set(__build_features__)


def test_previous_v2801_entrypoints_still_exist():
...
def test_current_scope_docs_distinguish_software_and_model_quality():
    docs = '\n'.join((ROOT/name).read_text() for name in (
        'TESTANLEITUNG_2.80.2.md', 'VERSION_2.80.2_BUILD_AND_TEST_REPORT.md'))
    for term in ('NOT_RUN', 'Force', 'v2.80.2-model-binding-native-quality-reporting',
...
def test_previous_closure_checks_are_all_carried_forward():
    ...
    previous = ast.parse((ROOT/'tests/test_v2801_release_closure.py').read_text())
    current = ast.parse((ROOT/'tests/test_v2802_release_closure.py').read_text())
...
def test_current_scope_document_is_allowlisted_by_every_verifier():
    ...
    relative = Path('docs/RELEASE_SCOPE_V2802.md')
    assert (ROOT/relative).is_file()
    assert build_source_manifest._source_candidate(ROOT/relative, ROOT, package_version='2.80.2')
    assert source_integrity._source_candidate(relative, package_version='2.80.2')
    updater = (ROOT/'scripts/update_source_release.sh').read_text()
