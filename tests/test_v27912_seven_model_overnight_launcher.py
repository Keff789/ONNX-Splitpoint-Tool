...
LAUNCHER = ROOT / "scripts/run_v27912_seven_model_long_overnight.sh"
PROFILE = ROOT / "profiles/complete_set_7models_v27912_b500_audit20.yaml"
...
def test_v27912_launcher_identity_profile_and_alias() -> None:
    assert os.access(LAUNCHER, os.X_OK)
    subprocess.run(["bash", "-n", str(LAUNCHER)], check=True)
    text = _text(LAUNCHER)
    profile_sha = hashlib.sha256(PROFILE.read_bytes()).hexdigest()
    assert 'expected = "2.79.12"' in text
    assert 'expected_build = "v2.79.12-platform-power-calibration-provenance-closure"' in text
    assert 'build = str(getattr(onnx_splitpoint_tool, "__build_id__", ""))' in text
    assert "V27912_RELEASE_IDENTITY=PASS" in text
    assert 'ACCEPTANCE="$TOOL/scripts/run_v27912_small_acceptance.sh"' in text
    assert 'STATUS_HELPER="$TOOL/scripts/launcher_status_v27912.py"' in text
    assert 'PROFILE="$TOOL/profiles/complete_set_7models_v27912_b500_audit20.yaml"' in text
    assert f"PROFILE_EXPECTED_SHA256={profile_sha}" in text
    assert 'profile.get("name") == "complete_set_7models_v27912_b500_audit20"' in text
    assert '"build_id": "v2.79.12-platform-power-calibration-provenance-closure"' in text
...
def test_v27912_launcher_retains_exact_historical_gate_receipts() -> None:
    text = _text(LAUNCHER)
    ...
    assert 'YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v27912_yolov7_claim_gate_32.py"' in text
    ...
    assert 'V27912_YOLOV7_CLAIM_GATE_32=PASS' in text


def test_v27912_launcher_preflight_is_non_dispatching_and_interlocked() -> None:
    text = _text(LAUNCHER)
    ...
    assert "V27912_SEVEN_MODEL_LONG_PREFLIGHT=PASS" in branch
    assert "LONG_RUN_STARTED=NO" in branch
    assert "nohup" not in branch
    assert "exit 0" in branch


def test_v27912_launcher_help_and_source_guard() -> None:
    ...
    assert "run_v27912_seven_model_long_overnight.sh" in help_result.stdout
...
def _load_current_fixture_module() -> object:
    path = ROOT / "tests" / "test_v27912_yolov7_claim_gate_32.py"
    spec = importlib.util.spec_from_file_location(
        "v27912_launcher_claim_fixture", path
    )
...
def _prepare_launcher_claim_fixture(paths: dict[str, object]) -> None:
    ...
    for name in (
        ...
        "verify_v27912_yolov7_claim_gate_32.py",
    ):
...
def _write_fake_tool_python(
    ...
    source = textwrap.dedent(
        ...
        if first.endswith("verify_v27912_yolov7_claim_gate_32.py"):
...
def test_v27912_launcher_preflight_executes_current_claim_verifier_end_to_end(
    ...
    for relative in (
        ...
        target.chmod(0o755)
    acceptance = source / "scripts" / "run_v27912_small_acceptance.sh"
    acceptance.write_text(
        "#!/usr/bin/env bash\nprintf 'V27912_TEST_ACCEPTANCE=PASS\\n'\n",
        encoding="utf-8",
    )
    ...
    assert "V27912_YOLOV7_CLAIM_GATE=PASS" in completed.stdout
    assert "V27912_YOLOV7_CLAIM_GATE_32=PASS" in completed.stdout
    assert "V27912_SEVEN_MODEL_LONG_PREFLIGHT=PASS" in completed.stdout
    assert "LONG_RUN_STARTED=NO" in completed.stdout
    controls = sorted(out_root.glob("v27912_7model_b500_audit20_launch_*"))
