...
SCRIPT = ROOT / "scripts" / "verify_v27910_yolov7_claim_gate_32.py"
RETAINED_V2796_HELPER = (
    ROOT
    / "tests/fixtures/v2796/yolov7_claim_gate_32/"
    "native_hailo_trt_concurrent_three_stage_from_benchmarkset.py"
)
SPEC = importlib.util.spec_from_file_location("v27910_yolov7_claim_gate", SCRIPT)
...
def _fixture(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    run_id = "v27910_yolov7_claim_gate_fixture"
    result = {
        "ok": True,
        ...
        "product_execution_context": {
            ...
            "source_snapshot_prefix": "ONNX-Splitpoint-Tool_v2.79.10",
...
def test_verifier_source_stays_python38_compatible() -> None:
    ...
    assert 'print("V27910_YOLOV7_CLAIM_GATE=PASS")' in text
...
def test_retained_v2796_helper_fixture_is_exact_and_normalizes_to_current() -> None:
    relative = "scripts/native_hailo_trt_concurrent_three_stage_from_benchmarkset.py"
    origin = gate.CARRY_FORWARD_ORIGIN
    retained = RETAINED_V2796_HELPER.read_bytes()
    current = (ROOT / relative).read_bytes()

    assert hashlib.sha256(retained).hexdigest() == origin[
        "snapshot_member_sha256"
    ][relative]
    assert hashlib.sha256(current).hexdigest() == origin[
        "current_snapshot_member_sha256"
    ][relative]
    assert gate._normalize_build_id_binding(
        retained, str(origin["build_id"]), "retained_v2796_helper"
    ) == gate._normalize_build_id_binding(
        current, gate.BUILD_ID, "current_v27910_helper"
    )


def _historical_v27910_snapshot_member(relative: str) -> bytes:
    """Reconstruct the verifier's frozen v2.79.10 current-source surface."""
    ...
    if relative == "onnx_splitpoint_tool/__init__.py":
        ...
        first_v27910_feature = (
            b'    "current_release_longrun_launcher_closure",\n'
        )
        assert payload.count(opener) == 1
        assert payload.count(first_v27910_feature) == 1
        start = payload.index(opener) + len(opener)
        retained = payload.index(first_v27910_feature, start)
        payload = payload[:start] + payload[retained:]
    elif relative == "onnx_splitpoint_tool/release_identity.py":
        assert payload.count(b'VERSION = "2.79.13"') == 1
        assert payload.count(
            b'BUILD_ID = "v2.79.13-platform-power-calibration-operational-repair"'
        ) == 1
        payload = payload.replace(
            b'VERSION = "2.79.13"', b'VERSION = "2.79.10"', 1
        ).replace(
            b'BUILD_ID = "v2.79.13-platform-power-calibration-operational-repair"',
            b'BUILD_ID = "v2.79.10-urecs-platform-power-release-closure"',
            1,
        )
        added_contracts = (
...
def _carry_forward_snapshot(
    ...
    origin = dict(gate.CARRY_FORWARD_ORIGIN)
    ...
    current_source = tmp_path / "frozen_v27910_source"
    archive_path = tmp_path / "source_snapshot.zip"
    with zipfile.ZipFile(archive_path, "w") as archive:
        for relative in gate.SNAPSHOT_MEMBERS:
            current = _historical_v27910_snapshot_member(relative)
            _write(current_source / relative, current)
            current_hashes[relative] = hashlib.sha256(current).hexdigest()
            captured = current
            if relative == "onnx_splitpoint_tool/release_identity.py":
                captured = captured.replace(
                    ('VERSION = "%s"' % gate.PACKAGE_VERSION).encode(),
                    ('VERSION = "%s"' % origin_version).encode(),
                ).replace(
                    ('BUILD_ID = "%s"' % gate.BUILD_ID).encode(),
                    ('BUILD_ID = "%s"' % origin_build).encode(),
                )
...
def test_carry_forward_snapshot_rejects_non_identity_helper_drift(
    ...
    archive, policy, current_source = _carry_forward_snapshot(
