...
from onnx_splitpoint_tool.v270d_smoke import (
    REQUIRED_FEATURES,
    main as smoke_main,
)
...
def test_v270d_identity_package_and_entrypoints_are_consistent() -> None:
    ...
    assert (
        'onnx-splitpoint-smoke-v270d = '
        '"onnx_splitpoint_tool.v270d_smoke:main"'
    ) in pyproject
    assert (
        'onnx-splitpoint-smoke-v2-70d = '
        '"onnx_splitpoint_tool.v270d_smoke:main"'
    ) in pyproject


def test_v270d_release_smoke_and_critical_modules(capsys) -> None:
    assert smoke_main() == 0
    result = json.loads(capsys.readouterr().out)
    assert result["schema"] == "onnx-splitpoint/v270d-smoke"
    ...
    assert {
        ...
        "resources/remote_scripts/native_hailo10_trt_e2e_from_benchmarkset.py",
...
def test_v270d_release_documents_record_current_identity() -> None:
    for path in (
        Path("README.md"),
