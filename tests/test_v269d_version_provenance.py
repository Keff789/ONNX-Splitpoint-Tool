...
from onnx_splitpoint_tool.v269d_smoke import REQUIRED_FEATURES, main as smoke_main
from onnx_splitpoint_tool.workflow.artifacts import package_build_snapshot
from onnx_splitpoint_tool.workflow.runner import WORKFLOW_VERSION


def test_v269d_identity_is_consistent() -> None:
    ...
    assert 'onnx-splitpoint-smoke-v269d = "onnx_splitpoint_tool.v269d_smoke:main"' in pyproject
    assert 'onnx-splitpoint-smoke-v2-69d = "onnx_splitpoint_tool.v269d_smoke:main"' in pyproject
    assert 'version = "2.75.47"' in Path("uv.lock").read_text(encoding="utf-8")


def test_v269d_docs_record_current_identity_and_scope() -> None:
    for path in (
        Path("README.md"),
...
def test_v269d_release_smoke(capsys) -> None:
    assert smoke_main() == 0
    assert '"ok": true' in capsys.readouterr().out


def test_v269d_build_snapshot_covers_repaired_claim_modules() -> None:
