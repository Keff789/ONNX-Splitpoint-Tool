...
PROFILE = (
    ROOT / "profiles" / "resnet50_v27538_deepx_full_quality_canary.yaml"
)
SETUP_ID = "orin_nx_deepx_m1_01"


def test_packaged_v27538_deepx_canary_raw_schema_and_load_contract(
    tmp_path: Path,
    monkeypatch,
) -> None:
    source = yaml.safe_load(PROFILE.read_text(encoding="utf-8"))

    assert source["name"] == "resnet50_v27538_deepx_full_quality_canary"
    assert "v2.75.38" in source["purpose"]
...
def test_packaged_v27538_deepx_canary_has_exact_effective_plan(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setenv(
