...
SCRIPT = ROOT / "scripts" / "verify_v27913_yolov7_claim_gate_32.py"
...
gate = _load(SCRIPT, "v27913_yolov7_claim_gate")
historical_fixture = _load(HISTORICAL_TEST, "v27913_historical_gate_fixture")


def build_current_claim_fixture(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> dict[str, Any]:
    """Reuse the exhaustive synthetic evidence builder with v2.79.13 IDs."""
...
def test_v27913_exact_fixture_passes_current_compatibility_verifier(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert receipt["verification_identity"] == {
        ...
        "verifier": "verify_v27913_yolov7_claim_gate_32.py",
...
def test_v27913_current_verifier_still_fails_closed_on_tampering(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    paths = build_current_claim_fixture(tmp_path, monkeypatch)
...
def test_v27913_frozen_historical_verifier_is_byte_pinned(tmp_path: Path) -> None:
...
def test_v27913_snapshot_member_pins_match_current_release() -> None:
    observed = {
        relative: hashlib.sha256((ROOT / relative).read_bytes()).hexdigest()
        for relative in gate.SNAPSHOT_MEMBERS
    }
    assert observed == gate.CARRY_FORWARD_ORIGIN[
        "current_snapshot_member_sha256"
    ]


def test_v27913_carry_forward_computational_surface_remains_exact() -> None:
...
def test_v27913_verifier_is_python38_compatible_and_executable() -> None:
    assert SCRIPT.stat().st_mode & 0o111
    text = SCRIPT.read_text(encoding="utf-8")
    assert 'print("V27913_YOLOV7_CLAIM_GATE=PASS")' in text
