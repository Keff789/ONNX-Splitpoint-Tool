...
from onnx_splitpoint_tool import v279_smoke, v282_smoke
...
def test_current_identity_and_smoke_alias() -> None:
    ...
    assert v282_smoke.VERSION == v279_smoke.VERSION == EXPECTED_VERSION
    assert v282_smoke.BUILD_ID == v279_smoke.BUILD_ID == EXPECTED_BUILD
...
def test_current_entrypoints_retain_historical_aliases() -> None:
    ...
    for alias in ("v282", "v2-82"):
        assert f'onnx-splitpoint-smoke-{alias} = "onnx_splitpoint_tool.v282_smoke:main"' in pyproject
    ...
    for path in ("scripts/run_v279_small_acceptance.sh", "scripts/run_local_acceptance.sh"):
        assert "run_v282_small_acceptance.sh" in _read(path)
...
def test_updater_exact_identity_and_retained_entrypoints() -> None:
    ...
    for alias in ("v282", "v2-82"):
        assert f"onnx-splitpoint-smoke-{alias}=onnx_splitpoint_tool.v282_smoke:main" in updater
...
def test_current_gate_retains_behavioral_regressions_not_old_identity_gate() -> None:
    gate = _read("scripts/run_v282_small_acceptance.sh")
    assert "tests/test_v282_release_closure.py" in _selected_gate_tests("scripts/run_v282_small_acceptance.sh")
    for test in (
...
def test_gate_covers_prior_selection_and_all_rev2_regressions() -> None:
    ...
    current = _selected_gate_tests("scripts/run_v282_small_acceptance.sh")
...
def test_required_acceptance_reports_never_pass_skipped_tests(tmp_path: Path, kind: str, expected_rc: int) -> None:
    ...
    required = tuple(sorted(path.stem for path in (ROOT / 'tests').glob('test_v282_*.py')))
...
spec = importlib.util.spec_from_file_location("current_full_smoke", root/"scripts/deepx_full_workflow_smoke_v282.py")
...
'''
    ...
    assert json.loads(result.stdout) == {
        "SOURCE_VERSION": "2.82", "SOURCE_TAG": "v282",
        "SOURCE_NAME": "ONNX-Splitpoint-Tool_v2.82_SOURCE.zip",
        "DEFAULT_BUNDLE": "ONNX-Splitpoint-Tool_v2.82_COMPLETE_DELIVERY_BUNDLE",
    }
...
def test_current_diagnostic_shell_launchers_are_packaged_and_parse(name: str) -> None:
    ...
    script = ROOT / 'scripts' / f'{name}_v282.sh'
    result = subprocess.run(['bash', '-n', str(script)], capture_output=True,
                            text=True, timeout=10)
    assert result.returncode == 0, result.stderr
    if name != 'run_hailo_gpu_compute':
        assert '_v282.py' in script.read_text()
...
def test_T32_R09_current_gate_preserves_every_previous_case_and_all_new_suites() -> None:
    ...
    previous = _selected_gate_tests("scripts/run_v2804_small_acceptance.sh")
    current_text = _read("scripts/run_v282_small_acceptance.sh")
    current = _selected_gate_tests("scripts/run_v282_small_acceptance.sh")
    ...
    assert "tests/test_v282_release_closure.py" in current
...
def test_T32_R09_scientific_scope_is_separate_from_software_release() -> None:
    docs = _read("TESTANLEITUNG_2.82.md")
...
def test_T33_gate_includes_every_current_suite_and_previous_behavior():
    gate = _read("scripts/run_v282_small_acceptance.sh")
    for path in (ROOT/"tests").glob("test_v282_*.py"):
        assert "tests/" + path.name in _selected_gate_tests("scripts/run_v282_small_acceptance.sh")
