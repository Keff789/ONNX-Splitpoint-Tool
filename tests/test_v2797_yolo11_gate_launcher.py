...
LAUNCHER = ROOT / "scripts/run_v2797_yolo11_r8b_gate.sh"
...
def test_v2797_gate_launcher_is_source_safe_and_has_valid_bash() -> None:
    ...
    assert "run_v2797_yolo11_r8b_gate.sh" in help_result.stdout
...
def test_standard_venv_python_symlink_passes_semantic_binding(
    ...
    recovery = tmp_path / "v2797_recovery_manifest.json"
    tool.mkdir()
    models.mkdir()
    recovery.write_text("{}\n", encoding="utf-8")
...
def test_launcher_pins_v2797_identity_and_all_prehardware_checks() -> None:
    source = _source()
    assert 'expected_version = "2.79.7"' in source
    assert f'expected_build = "{BUILD_ID}"' in source
    assert 'PROFILE="$TOOL/profiles/yolo11l_v2797_r8b_full_b067_gate.yaml"' in source
    assert 'VERIFIER="$TOOL/scripts/verify_v2797_yolo11_r8b_gate.py"' in source
    assert (
        "PROFILE_SHA256="
        "69349eb6d84b59be5fc0bd5316b7eb5204d3f672dde2788bb5030702c9ed4ad7"
        in source
    )
    assert "V2797_R8B_RELEASE_IDENTITY=PASS" in source
    assert "V2797_R8B_SOURCE_MANIFEST=PASS" in source
    assert "V2797_YOLO11_R8B_GATE=PASS" in source
    ...
    assert 'RECOVERY_MANIFEST="$GATE_OUTPUT/v2797_recovery_manifest.json"' in source
    assert "os.O_EXCL" in source
    assert "O_NOFOLLOW" in source

    release = source.index("V2797_R8B_RELEASE_IDENTITY=PASS")
    manifest = source.index("V2797_R8B_SOURCE_MANIFEST=PASS")
...
def test_heredocs_are_not_part_of_an_or_list_and_errors_fail_closed() -> None:
    ...
    assert source.count("\nPY\nthen\n") == 3
    assert 'die 65 "Installierte v2.79.7-Package-/Workflow-Identität stimmt nicht"' in source
