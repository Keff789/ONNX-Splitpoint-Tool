...
LAUNCHER = ROOT / "scripts/run_v27910_seven_model_long_overnight.sh"
GENERIC_LAUNCHER = ROOT / "scripts/run_v279_seven_model_long_overnight.sh"
STATUS_HELPER = ROOT / "scripts/launcher_status_v27910.py"
...
def test_v27910_launcher_is_source_safe_helpful_and_executable() -> None:
    assert os.access(LAUNCHER, os.X_OK)
    subprocess.run(["bash", "-n", str(LAUNCHER)], check=True)

    help_result = _run("--help")
    assert help_result.returncode == 0
    assert "run_v27910_seven_model_long_overnight.sh" in help_result.stdout
...
def test_r8b_output_option_fails_closed_before_host_preflight() -> None:
    ...
    unsupported = _run("--unsupported-v27910-option")
    assert unsupported.returncode == 64
    assert "unsupported option" in unsupported.stderr


def test_v27910_launcher_pins_release_profile_identity_and_acceptance() -> None:
    text = _read(LAUNCHER)
    assert 'expected = "2.79.10"' in text
    assert "V27910_RELEASE_IDENTITY=PASS" in text
    assert 'ACCEPTANCE="$TOOL/scripts/run_v27910_small_acceptance.sh"' in text
    assert 'STATUS_HELPER="$TOOL/scripts/launcher_status_v27910.py"' in text
    assert 'PROFILE="$TOOL/profiles/complete_set_7models_v27910_b500_audit20.yaml"' in text
    assert (
        "PROFILE_EXPECTED_SHA256="
        "65c926e7da85d1a46ce099e774c20c80e12e3340684dc5d271038957d69022f9"
    ) in text
    assert 'profile.get("name") == "complete_set_7models_v27910_b500_audit20"' in text
...
def test_launcher_holds_shared_platform_interlock_across_detached_run() -> None:
    text = _read(LAUNCHER)
    interlock_path = (
        'PLATFORM_INTERLOCK="$PLATFORM_LOCK_PARENT/'
        'workflow_platform_interlock.lock"'
    )
    assert interlock_path in text
    foreground_open = text.index('exec 7<>"$PLATFORM_INTERLOCK"')
    foreground_lock = text.index("flock -n -s 7", foreground_open)
    installed_source_read = text.index('[[ -d "$TOOL"', foreground_lock)
    detached = text.index("nohup setsid --fork --wait bash -c", installed_source_read)
    inherited_lock = text.index("if ! flock -n -s 7; then", detached)
    campaign_lock = text.index('exec 9>"$lock"', inherited_lock)
    workflow = text.index(
        'bash "$tool/scripts/run_fresh_standard_workflow.sh"',
        campaign_lock,
    )
    assert foreground_open < foreground_lock < installed_source_read
    assert detached < inherited_lock < campaign_lock < workflow
    assert "exclusive source updater interlock is active" in text
...
def test_r8b_receipt_is_exact_and_precedes_every_launch_path() -> None:
    ...
    assert 'RECOVERY_HELPER="$TOOL/scripts/prepare_v27910_yolo11_r8b_recovery.py"' in text
    ...
    assert text.count('"version": "2.79.10"') >= 3
    assert text.count(
    ...
    assert 'gate.get("version") != "2.79.10"' in text
    for marker in (
    ...
    gate_pass = text.index('print("V27910_YOLO11_FULL_GATE=PASS")', verify_call)
...
def test_yolov7_claim_gate_is_strict_receipt_bound_and_pre_dispatch() -> None:
    text = _read(LAUNCHER)
    assert 'YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v27910_yolov7_claim_gate_32.py"' in text
    ...
    pass_marker = text.index('print("V27910_YOLOV7_CLAIM_GATE_32=PASS")', verifier)
...
def test_preflight_is_receipt_bound_and_cannot_dispatch() -> None:
    ...
    assert "V27910_SEVEN_MODEL_LONG_PREFLIGHT=PASS" in branch
    assert "V27910_YOLO11_FULL_GATE=PASS" in branch
    assert "V27910_YOLOV7_CLAIM_GATE_32=PASS" in branch
