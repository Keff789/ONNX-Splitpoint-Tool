...
from onnx_splitpoint_tool import v279_smoke, v2804_smoke
...
EXPECTED_VERSION = "2.80.4"
EXPECTED_BUILD = "v2.80.4-hailo8-context-quality-cancel-complete-debug"
...
def test_current_identity_and_smoke_alias() -> None:
    ...
    assert v2804_smoke.VERSION == v279_smoke.VERSION == EXPECTED_VERSION
    assert v2804_smoke.BUILD_ID == v279_smoke.BUILD_ID == EXPECTED_BUILD


def test_current_entrypoints_retain_historical_aliases() -> None:
    pyproject = _read("pyproject.toml")
    assert 'version = "2.80.4"' in pyproject
    assert 'name = "onnx-splitpoint-tool"\nversion = "2.80.4"' in _read("uv.lock")
    ...
    for path in ("scripts/run_v279_small_acceptance.sh", "scripts/run_local_acceptance.sh"):
        assert "run_v2804_small_acceptance.sh" in _read(path)
...
def test_updater_exact_identity_and_retained_entrypoints() -> None:
    ...
    for marker in (
        ...
        "ONNX-Splitpoint-Tool_v2.80.4",
        "--expected-version 2.80.4",
        "--run-entrypoint onnx-splitpoint-smoke-v2804",
    ):
    ...
    for alias in ("v2804", "v2-80-4"):
        assert f"onnx-splitpoint-smoke-{alias}=onnx_splitpoint_tool.v2804_smoke:main" in updater
...
def test_release_docs_are_current_and_historical_docs_are_retained() -> None:
    for path in ("TESTANLEITUNG_2.80.4.md", "VERSION_2.80.4_BUILD_AND_TEST_REPORT.md"):
        contents = _read(path)
...
def test_current_gate_retains_behavioral_regressions_not_old_identity_gate() -> None:
    gate = _read("scripts/run_v2804_small_acceptance.sh")
    assert "tests/test_v2804_release_closure.py" in _selected_gate_tests("scripts/run_v2804_small_acceptance.sh")
    for test in (
...
def test_current_hardware_independent_smoke(capsys) -> None:
    assert v2804_smoke.main() == 0
    assert "PASS v2.80.4 smoke" in capsys.readouterr().out
...
assert modules[0].VERSION == "2.80.4"
...
'''
    ...
    assert json.loads(result.stdout.strip().splitlines()[-1])["version"] == "2.80.4"
...
def test_gate_covers_prior_selection_and_all_rev2_regressions() -> None:
    ...
    previous = _selected_gate_tests("scripts/run_v2803_small_acceptance.sh")
    current = _selected_gate_tests("scripts/run_v2804_small_acceptance.sh")
    previous.remove("tests/test_v2803_release_closure.py")
...
def test_exact_recorded_npz_is_manifested_and_other_binaries_stay_excluded(tmp_path: Path) -> None:
    ...
    updater = _read("scripts/update_source_release.sh")
    embedded = updater[updater.index('EXPECTED_VERSION = "2.80.4"'):]
    predicate = embedded[embedded.index("def valid_logical_path("):embedded.index("def strict_sha256(")]
    ns = {"PurePosixPath": PurePosixPath, "EXPECTED_VERSION": "2.80.4"}
    for name in (
...
def test_required_acceptance_reports_never_pass_skipped_tests(tmp_path: Path, kind: str, expected_rc: int) -> None:
    ...
    required = tuple(sorted(path.stem for path in (ROOT / 'tests').glob('test_v2804_*.py')))
...
spec = importlib.util.spec_from_file_location("current_full_smoke", root/"scripts/deepx_full_workflow_smoke_v2804.py")
...
'''
    ...
    assert json.loads(result.stdout) == {
        "SOURCE_VERSION": "2.80.4", "SOURCE_TAG": "v2804",
        "SOURCE_NAME": "ONNX-Splitpoint-Tool_v2.80.4_SOURCE.zip",
        "DEFAULT_BUNDLE": "ONNX-Splitpoint-Tool_v2.80.4_COMPLETE_DELIVERY_BUNDLE",
    }
...
def test_T32_R09_current_launchers_parse_in_fresh_isolated_process(name: str, tmp_path: Path) -> None:
    ...
    result = subprocess.run([sys.executable, "-I", "-B", str(ROOT / "scripts" / f"{name}_v2804.py"), "--help"],
                            cwd=tmp_path, text=True, capture_output=True, timeout=45,
                            env={k: v for k, v in os.environ.items() if k not in {"PYTHONPATH", "PYTHONHOME"}})
    assert result.returncode == 0, result.stdout + result.stderr
...
def test_current_diagnostic_shell_launchers_are_packaged_and_parse(name: str) -> None:
    ...
    script = ROOT / 'scripts' / f'{name}_v2804.sh'
    result = subprocess.run(['bash', '-n', str(script)], capture_output=True,
                            text=True, timeout=10)
    assert result.returncode == 0, result.stderr
    if name != 'run_hailo_gpu_compute':
        assert '_v2804.py' in script.read_text()
...
def test_T32_R09_current_gate_preserves_every_previous_case_and_all_new_suites() -> None:
    ...
    previous = _selected_gate_tests("scripts/run_v2803_small_acceptance.sh")
    current_text = _read("scripts/run_v2804_small_acceptance.sh")
    current = _selected_gate_tests("scripts/run_v2804_small_acceptance.sh")
    assert previous - {"tests/test_v2803_release_closure.py"} <= current
    ...
    assert "tests/test_v2804_release_closure.py" in current
...
def test_T32_R09_scientific_scope_is_separate_from_software_release() -> None:
    docs = _read("TESTANLEITUNG_2.80.4.md")
...
def test_T33_gate_includes_every_current_suite_and_previous_behavior():
    gate = _read("scripts/run_v2804_small_acceptance.sh")
    for path in (ROOT/"tests").glob("test_v2804_*.py"):
        assert "tests/" + path.name in _selected_gate_tests("scripts/run_v2804_small_acceptance.sh")
