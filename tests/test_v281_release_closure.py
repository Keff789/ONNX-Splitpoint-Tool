...
from onnx_splitpoint_tool import v279_smoke, v281_smoke
...
EXPECTED_VERSION = "2.81"
EXPECTED_BUILD = "v2.81-hailo-cold-preflight-shared-input-layout-terminal-status"
...
def _selected_gate_tests(relative: str) -> set[str]:
    """Observe actual Bash expansion of the gate's pytest argument lists.

    Replacing only the executable with an argv recorder preserves literal
    node selections and unquoted globs without executing any tests or gates.
    """
    import subprocess

    commands = []
    lines = iter(_read(relative).splitlines())
    for line in lines:
        if not line.startswith('"$PYTHON" -B -m pytest '):
            continue
        parts = [line]
        while parts[-1].rstrip().endswith('\\'):
            parts.append(next(lines))
        commands.append('\n'.join(parts))
    assert commands, f"No pytest invocations found in {relative}"
    script = """set -eu
PYTHON=record_arguments
PYTEST_JUNIT_MAIN=main.xml
PYTEST_JUNIT_GUI=gui.xml
record_arguments() { printf '%s\\0' "$@"; }
""" + "\n".join(commands)
    captured = subprocess.run(['bash', '-c', script], cwd=ROOT,
                              capture_output=True, timeout=10, check=True)
    selected = {argument.decode() for argument in captured.stdout.split(b'\0')
                if argument.startswith(b'tests/')}
    assert selected, relative
    assert all((ROOT / item.split('::', 1)[0]).is_file() for item in selected), selected
    return selected
...
def test_current_identity_and_smoke_alias() -> None:
    ...
    assert v281_smoke.VERSION == v279_smoke.VERSION == EXPECTED_VERSION
    assert v281_smoke.BUILD_ID == v279_smoke.BUILD_ID == EXPECTED_BUILD


def test_current_entrypoints_retain_historical_aliases() -> None:
    pyproject = _read("pyproject.toml")
    assert 'version = "2.81"' in pyproject
    assert 'name = "onnx-splitpoint-tool"\nversion = "2.81"' in _read("uv.lock")
    ...
    for alias in ("v281", "v2-81"):
        assert f'onnx-splitpoint-smoke-{alias} = "onnx_splitpoint_tool.v281_smoke:main"' in pyproject
    for alias in ("v2804", "v2-80-4"):
        assert f'onnx-splitpoint-smoke-{alias} = "onnx_splitpoint_tool.v2804_smoke:main"' in pyproject
    for alias in ("v2803", "v2-80-3"):
        assert f'onnx-splitpoint-smoke-{alias} = "onnx_splitpoint_tool.v2803_smoke:main"' in pyproject
    for path in ("scripts/run_v279_small_acceptance.sh", "scripts/run_local_acceptance.sh"):
        assert "run_v281_small_acceptance.sh" in _read(path)
...
def test_updater_exact_identity_and_retained_entrypoints() -> None:
    ...
    for marker in (
        ...
        "ONNX-Splitpoint-Tool_v2.81",
        "--expected-version 2.81",
        "--run-entrypoint onnx-splitpoint-smoke-v281",
    ):
    ...
    for alias in ("v281", "v2-81"):
        assert f"onnx-splitpoint-smoke-{alias}=onnx_splitpoint_tool.v281_smoke:main" in updater
...
def test_release_docs_are_current_and_historical_docs_are_retained() -> None:
    for path in ("TESTANLEITUNG_2.81.md", "VERSION_2.81_BUILD_AND_TEST_REPORT.md"):
        contents = _read(path)
...
def test_current_gate_retains_behavioral_regressions_not_old_identity_gate() -> None:
    gate = _read("scripts/run_v281_small_acceptance.sh")
    assert "tests/test_v281_release_closure.py" in _selected_gate_tests("scripts/run_v281_small_acceptance.sh")
    for test in (
...
def test_current_hardware_independent_smoke(capsys) -> None:
    assert v281_smoke.main() == 0
    assert "PASS v2.81 smoke" in capsys.readouterr().out
...
'''
    ...
    assert json.loads(result.stdout.strip().splitlines()[-1])["version"] == "2.81"
...
def test_gate_covers_prior_selection_and_all_rev2_regressions() -> None:
    ...
    previous = _selected_gate_tests("scripts/run_v2804_small_acceptance.sh")
    current = _selected_gate_tests("scripts/run_v281_small_acceptance.sh")
    previous.remove("tests/test_v2804_release_closure.py")
...
def test_exact_recorded_npz_is_manifested_and_other_binaries_stay_excluded(tmp_path: Path) -> None:
    ...
    updater = _read("scripts/update_source_release.sh")
    embedded = updater[updater.index('EXPECTED_VERSION = "2.81"'):]
    predicate = embedded[embedded.index("def valid_logical_path("):embedded.index("def strict_sha256(")]
    ns = {"PurePosixPath": PurePosixPath, "EXPECTED_VERSION": "2.81"}
    for name in (
...
def test_required_acceptance_reports_never_pass_skipped_tests(tmp_path: Path, kind: str, expected_rc: int) -> None:
    ...
    required = tuple(sorted(path.stem for path in (ROOT / 'tests').glob('test_v281_*.py')))
    assert required, 'current release has no required tests'
    cases = "".join(f'<testcase classname="tests.{name}" name="ok"/>' for name in required)
    valid = f'<testsuites><testsuite tests="{len(required)}" failures="0" errors="0" skipped="0">{cases}</testsuite></testsuites>'
...
spec = importlib.util.spec_from_file_location("current_full_smoke", root/"scripts/deepx_full_workflow_smoke_v281.py")
...
'''
    ...
    assert json.loads(result.stdout) == {
        "SOURCE_VERSION": "2.81", "SOURCE_TAG": "v281",
        "SOURCE_NAME": "ONNX-Splitpoint-Tool_v2.81_SOURCE.zip",
        "DEFAULT_BUNDLE": "ONNX-Splitpoint-Tool_v2.81_COMPLETE_DELIVERY_BUNDLE",
    }
...
def test_T32_R09_current_launchers_parse_in_fresh_isolated_process(name: str, tmp_path: Path) -> None:
    ...
    result = subprocess.run([sys.executable, "-I", "-B", str(ROOT / "scripts" / f"{name}_v281.py"), "--help"],
                            cwd=tmp_path, text=True, capture_output=True, timeout=45,
                            env={k: v for k, v in os.environ.items() if k not in {"PYTHONPATH", "PYTHONHOME"}})
    assert result.returncode == 0, result.stdout + result.stderr
...
@pytest.mark.parametrize('name', (
    'run_hailo_gpu_compute', 'run_classification_input_probe',
    'run_hailo10_yolo26_boundary_probe',
))
def test_current_diagnostic_shell_launchers_are_packaged_and_parse(name: str) -> None:
    import subprocess

    script = ROOT / 'scripts' / f'{name}_v281.sh'
    result = subprocess.run(['bash', '-n', str(script)], capture_output=True,
                            text=True, timeout=10)
    assert result.returncode == 0, result.stderr
    if name != 'run_hailo_gpu_compute':
        assert '_v281.py' in script.read_text()
        assert '_v2803.py' not in script.read_text()
...
def test_T32_R09_current_gate_preserves_every_previous_case_and_all_new_suites() -> None:
    ...
    previous = _selected_gate_tests("scripts/run_v2804_small_acceptance.sh")
    current_text = _read("scripts/run_v281_small_acceptance.sh")
    current = _selected_gate_tests("scripts/run_v281_small_acceptance.sh")
    assert previous - {"tests/test_v2804_release_closure.py"} <= current
    assert "tests/test_v27934_release_closure.py" not in current
    ...
    assert "tests/test_v281_release_closure.py" in current
...
def test_T32_R09_scientific_scope_is_separate_from_software_release() -> None:
    docs = _read("TESTANLEITUNG_2.81.md")
    assert all(term in docs for term in ("B500", "NOT_RUN", "Statusachsen", "Energie"))
    historical = _read("TESTANLEITUNG_2.80.3.md")
    assert all(term in historical for term in ("1 s × 3", "30 s × 3", "60 s × 3", "FS/command", "B500", "5.000", "NOT_RUN"))
...
def _exercise_canonical_workdir_updater(tmp_path: Path, updater_root: Path) -> dict:
    ...
    write_package(target, '2.80.3', 'v2.80.3-build-readiness-native-not-started-debug-export')
    ...
    run([str(py), '-I', '-B', str(target/'scripts/refresh_editable_install.py'), '--root', str(target), '--expected-version', '2.80.3'])
    payloads = {
...
def test_T33_gate_includes_every_current_suite_and_previous_behavior():
    gate = _read("scripts/run_v281_small_acceptance.sh")
    for path in (ROOT/"tests").glob("test_v281_*.py"):
        assert "tests/" + path.name in _selected_gate_tests("scripts/run_v281_small_acceptance.sh")
