...
@pytest.mark.parametrize("version", ["2797", "2798"])
...
def _v2798_payload() -> dict:
    payload = yaml.safe_load(_profile("2798").read_text(encoding="utf-8"))
...
def test_yolo11_recovery_policy_schema_is_fail_closed(mutation: str) -> None:
    payload = _v2798_payload()
    policy = payload["v2798_yolo11_recovery"]
    ...
    with pytest.raises(ValueError, match="Invalid evaluation profile"):
        validate_evaluation_profile_payload(
            payload,
            source=f"v2798-yolo11-recovery-negative:{mutation}",
        )
