...
from onnx_splitpoint_tool import v279_smoke, v27925_smoke
...
EXPECTED_VERSION = "2.79.25"
EXPECTED_BUILD = "v2.79.25-deepx-gpu-quality-followup"
...
def test_current_identity_and_smoke_alias() -> None:
    ...
    assert v27925_smoke.VERSION == v279_smoke.VERSION == EXPECTED_VERSION
    assert v27925_smoke.BUILD_ID == v279_smoke.BUILD_ID == EXPECTED_BUILD


def test_current_entrypoints_retain_historical_aliases() -> None:
    pyproject = _read("pyproject.toml")
    assert 'version = "2.79.25"' in pyproject
    assert 'name = "onnx-splitpoint-tool"\nversion = "2.79.25"' in _read("uv.lock")
    for number in (19, 20, 21, 22, 23, 24, 25):
    ...
    for path in ("scripts/run_v279_small_acceptance.sh", "scripts/run_local_acceptance.sh"):
        assert "run_v27925_small_acceptance.sh" in _read(path)
...
def test_updater_exact_identity_and_retained_entrypoints() -> None:
    ...
    for marker in (
        ...
        "ONNX-Splitpoint-Tool_v2.79.25",
        "--expected-version 2.79.25",
        "--run-entrypoint onnx-splitpoint-smoke-v27925",
    ):
...
def test_release_docs_are_current_and_historical_docs_are_retained() -> None:
    for path in ("TESTANLEITUNG_2.79.25.md", "VERSION_2.79.25_BUILD_AND_TEST_REPORT.md"):
        contents = _read(path)
...
def test_current_gate_retains_behavioral_regressions_not_old_identity_gate() -> None:
    gate = _read("scripts/run_v27925_small_acceptance.sh")
    assert "tests/test_v27925_release_closure.py" in gate
    for test in (
...
def test_current_hardware_independent_smoke(capsys) -> None:
    assert v27925_smoke.main() == 0
    assert "PASS v2.79.25 smoke" in capsys.readouterr().out
