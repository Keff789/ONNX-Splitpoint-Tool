...
def _make_model(tmp_path: Path) -> tuple[Path, object]:
    x = helper.make_tensor_value_info('x', TensorProto.FLOAT, [1, 1, 4, 4])
    y = helper.make_tensor_value_info('y', TensorProto.FLOAT, [1, 1, 4, 4])
    nodes = [
        helper.make_node('Identity', ['x'], ['pixel_values_QuantizeLinear'], name='pixel_values_QuantizeLinear'),
        helper.make_node('Identity', ['pixel_values_QuantizeLinear'], ['mid'], name='/convnext/embeddings/patch_embeddings/Conv_quant'),
        helper.make_node('Identity', ['mid'], ['y'], name='tail'),
    ]
    model = helper.make_model(helper.make_graph(nodes, 'g', [x], [y]), opset_imports=[helper.make_opsetid('', 13)])
    model_path = tmp_path / 'toy.onnx'
    onnx.save(model, model_path)
    return model_path, model


def _make_exec_cfg(tmp_path: Path, model: object, runtime: BenchmarkGenerationRuntime) -> BenchmarkGenerationExecutionConfig:
...
def test_plan_aware_preflight_keeps_trt_to_hailo_for_input_side_unsupported_ops(tmp_path: Path) -> None:
    model_path, model = _make_model(tmp_path)
    runtime = _make_runtime(tmp_path)
    exec_cfg = _make_exec_cfg(tmp_path, model, runtime)
    ...
    run_ids = [str(r.get('id')) for r in list(exec_service.last_cfg.bench_plan_runs or []) if isinstance(r, dict)]
    assert 'trt_to_hailo8' in run_ids
    assert 'hailo8_to_trt' not in run_ids

    hailo_run = next(r for r in exec_service.last_cfg.bench_plan_runs if isinstance(r, dict) and str(r.get('id')) == 'hailo8')
    ...
    assert any('plan-aware Hailo preflight adjustment' in line for line in list(result.summary_data.get('plan_adjustments') or []))


def test_plan_aware_preflight_still_aborts_when_no_runs_remain(tmp_path: Path) -> None:
    model_path, model = _make_model(tmp_path)
    runtime = _make_runtime(tmp_path)
    exec_cfg = _make_exec_cfg(tmp_path, model, runtime)
    exec_cfg = BenchmarkGenerationExecutionConfig(**{**exec_cfg.__dict__, 'bench_plan_runs': []})
