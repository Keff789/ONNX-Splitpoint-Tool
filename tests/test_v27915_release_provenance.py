...
from onnx_splitpoint_tool import v27915_smoke, v279_smoke
...
def test_v27915_exact_identity_and_current_alias() -> None:
    assert VERSION == RELEASE == package.__version__ == package.__release__ == "2.79.15"
    ...
    for removed in v27915_smoke.REMOVED_IDLE_CALIBRATION_FEATURES:
        assert removed not in package.__build_features__
    assert v279_smoke.VERSION == v27915_smoke.VERSION == VERSION
    assert v279_smoke.BUILD_ID == v27915_smoke.BUILD_ID == BUILD_ID
    assert v279_smoke.main is v27915_smoke.main
    assert "from .v27915_smoke import" in _text(
        "onnx_splitpoint_tool/v279_smoke.py"
    )


def test_v27915_packaging_updater_and_aliases() -> None:
    ...
    for marker in (
        ...
        "onnx-splitpoint-tool==2.79.15",
    ):
        assert marker in updater
    assert 'run_v27915_small_acceptance.sh" "$@"' in _text(
        "scripts/run_v279_small_acceptance.sh"
    )
    assert 'run_v27915_small_acceptance.sh' in _text(
        "scripts/run_local_acceptance.sh"
    )
    assert 'run_v27915_seven_model_long_overnight.sh' in _text(
        "scripts/run_v279_seven_model_long_overnight.sh"
    )


def test_v27915_calibration_contract_is_direct_and_unsealed() -> None:
...
def test_v27915_acceptance_and_smoke(capsys) -> None:
    for relative in (
        "scripts/run_v27915_small_acceptance.sh",
        "scripts/run_v27915_seven_model_long_overnight.sh",
        "scripts/launcher_status_v27915.py",
        "scripts/verify_v27915_yolov7_claim_gate_32.py",
        "profiles/complete_set_7models_v27915_b500_audit20.yaml",
    ...
    acceptance = _text("scripts/run_v27915_small_acceptance.sh")
    assert '--root "$SOURCE_ROOT" --verify --scope installed' in acceptance
    assert v27915_smoke.main() == 0
    assert "PASS v2.79.15 smoke" in capsys.readouterr().out


def test_v27915_longrun_profile_and_snapshot_pins_match_release_files() -> None:
    profile = ROOT / "profiles/complete_set_7models_v27915_b500_audit20.yaml"
    launcher = _text("scripts/run_v27915_seven_model_long_overnight.sh")
    profile_sha = hashlib.sha256(profile.read_bytes()).hexdigest()
    assert f"PROFILE_EXPECTED_SHA256={profile_sha}" in launcher

    verifier = _text("scripts/verify_v27915_yolov7_claim_gate_32.py")
    for relative in (
        ...
        assert f'"{current_sha}"' in verifier
