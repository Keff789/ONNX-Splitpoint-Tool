...
SCRIPT = ROOT / "scripts" / "verify_v27912_yolov7_claim_gate_32.py"
...
gate = _load(SCRIPT, "v27912_yolov7_claim_gate")
historical_fixture = _load(HISTORICAL_TEST, "v27912_historical_gate_fixture")


def build_current_claim_fixture(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> dict[str, Any]:
    """Reuse the exhaustive synthetic evidence builder with v2.79.12 IDs."""
...
def test_v27912_exact_fixture_passes_current_compatibility_verifier(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert receipt["verification_identity"] == {
        ...
        "verifier": "verify_v27912_yolov7_claim_gate_32.py",
...
def test_v27912_current_verifier_still_fails_closed_on_tampering(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    paths = build_current_claim_fixture(tmp_path, monkeypatch)
...
def test_v27912_frozen_historical_verifier_is_byte_pinned(tmp_path: Path) -> None:
...
def test_v27912_snapshot_member_pins_remain_historically_frozen() -> None:
    assert gate.CARRY_FORWARD_ORIGIN["current_snapshot_member_sha256"] == {
        "onnx_splitpoint_tool/__init__.py": (
            "ca50bf806d451f7bba4f9709d7080037fbc6d22101a8e965e3c472c9cc14729d"
        ),
        "onnx_splitpoint_tool/release_identity.py": (
            "27a9bb5bfcf84a07e2403036210eaadef16c47d44a0481a4dd298ecef619f788"
        ),
...
def test_v27912_carry_forward_computational_surface_remains_exact() -> None:
...
def test_v27912_verifier_is_python38_compatible_and_executable() -> None:
    assert SCRIPT.stat().st_mode & 0o111
    text = SCRIPT.read_text(encoding="utf-8")
    assert 'print("V27912_YOLOV7_CLAIM_GATE=PASS")' in text
