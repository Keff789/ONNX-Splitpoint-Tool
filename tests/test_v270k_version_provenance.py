...
EXPECTED_CRITICAL_MODULES = {
    "v270k_smoke.py",
    "validation/host_postprocess.py",
...
def test_v270k_identity_is_consistent() -> None:
...
def test_v270k_package_metadata_and_entrypoints_are_consistent() -> None:
    ...
    assert (
        'onnx-splitpoint-smoke-v270k = '
        '"onnx_splitpoint_tool.v270k_smoke:main"'
    ) in pyproject
    assert (
        'onnx-splitpoint-smoke-v2-70k = '
        '"onnx_splitpoint_tool.v270k_smoke:main"'
    ) in pyproject


def test_v270k_declares_all_repair_features() -> None:
    assert EXPECTED_FEATURES.issubset(set(__build_features__))


def test_v270k_release_smoke_and_critical_modules(capsys) -> None:
    smoke = importlib.import_module("onnx_splitpoint_tool.v270k_smoke")
    assert EXPECTED_FEATURES.issubset(set(smoke.REQUIRED_FEATURES))

    assert smoke.main() == 0
    result = json.loads(capsys.readouterr().out)
    assert result["schema"] == "onnx-splitpoint/v270k-smoke"
    assert result["ok"] is True
    assert result["passed"] == 11
    assert result["failed"] == 0
    assert (
        result["checks"]["native_contract_completion_repairs"]
        is True
    )
    assert result["repair_contracts"] == {
        "sealed_rank3_full_self_reference_feed": True,
        "native_single_input_reject_backfill": True,
        "canonical_host_postprocessing_alias": True,
        "backend_independent_completed_task_endpoint": True,
    }
...
def test_v270k_source_console_smoke_selects_current_regressions() -> None:
    ...
    for test_path in (
        ...
        "tests/test_v270k_completed_endpoint_host_tail.py",
        "tests/test_v270k_version_provenance.py",
...
def test_v270k_release_documents_record_current_identity(
    path: Path,
) -> None:
