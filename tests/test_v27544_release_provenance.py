...
from onnx_splitpoint_tool.v27544_smoke import (
    BUILD_ID,
    CRITICAL_MODULES,
    CURRENT_BUILD_ID,
    CURRENT_VERSION,
...
def test_v27544_exact_release_identity_and_features() -> None:
    assert VERSION == "2.75.44"
    assert BUILD_ID == "v2.75.44-runtime-validation-cohort-projection"
...
def test_v27544_packaging_documents_profile_and_harness_are_pinned() -> None:
    ...
    harness = ROOT / "scripts/run_v27544_small_acceptance.sh"
...
def test_v27544_claim_critical_inventory_covers_retained_repairs() -> None:
...
def test_v27544_preserves_v27543_historical_assets() -> None:
    for name in (
        "onnx_splitpoint_tool/v27543_smoke.py",
        ...
        assert "2.75.43" in path.read_text(encoding="utf-8")


def test_v27544_source_allowlist_retains_v27543_release_docs() -> None:
    helper = (
...
def test_v27544_hardware_independent_smoke(capsys) -> None:
    assert smoke_main() == 0
    result = json.loads(capsys.readouterr().out)
    assert result["schema"] == "onnx-splitpoint/v27544-smoke"
