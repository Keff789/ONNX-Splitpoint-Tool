"""Hardware-independent release smoke for the v2.81 required build readiness, explicit Native non-starts and bounded debug export.
...
NEW_FEATURES = {'v281_shared_native_rgb_input', 'v281_quality_identity_normalization', 'v281_exact_negative_terminal_projection', 'v281_hailo10_physical_layout', 'v281_pending_cold_compiler_preflight'}


from .v2804_smoke import REQUIRED_FEATURES as PREVIOUS_REQUIRED_FEATURES
...
def main() -> int:
    ...
    checks = {
        "version": __version__ == __release__ == VERSION == "2.81",
        "lineage": __development_lineage__ == LINEAGE == "v2.79",
        "build": __build_id__ == WORKFLOW_VERSION == BUILD_ID
        == "v2.81-hailo-cold-preflight-shared-input-layout-terminal-status",
        ...
        "current_aliases": (
            "from .v281_smoke import" in
            (root / "onnx_splitpoint_tool/v279_smoke.py").read_text(encoding="utf-8")
            and all("run_v281_small_acceptance.sh" in
