    ...
    return {
        ...
        "build": {
            ...
            "hailo": {
                ...
                "build_part1": True,
                "build_part2": True,
                # Normal preparation reuses compatible artifacts; private
                # diagnostic cold builds use a separate nonpublishing call.
                "force_build": False,
...
def _migrate_run_modes_config_v60p(payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if smoke is not None:
        ...
        # new four-worker default. Explicit saves persist this migration;
        # read-only previews resolve it without rewriting the registry.
        if version < 8:
...
def _validate_mode_types(payload: Mapping[str, Any], template: Mapping[str, Any], *, field: str) -> None:
    """Validate supplied structures and boolean leaves before merging defaults."""
    for key, value in payload.items():
        reference = template.get(key)
        location = f"{field}.{key}"
        if type(reference) is bool:
            parse_config_bool(value, field=location)
        elif isinstance(reference, Mapping):
            if not isinstance(value, Mapping):
                raise ValueError(f"config_mapping_invalid:{location}")
            _validate_mode_types(value, reference, field=location)


_LEGACY_FINAL_DESCRIPTIONS = {
    "Maximum-effort frozen evaluated-matrix campaign with 5,000 validation items per task, strict provenance and thesis reporting.",
    "Maximum-effort frozen campaign with 5,000 validation items per task by default, strict provenance and thesis reporting.",
}
...
def validate_run_modes_config(payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if "modes" in payload:
        supplied_modes = payload["modes"]
        if not isinstance(supplied_modes, Mapping):
            raise ValueError("config_mapping_invalid:modes")
        for mode_id, mode in supplied_modes.items():
            if not isinstance(mode, Mapping):
                raise ValueError(f"config_mapping_invalid:modes.{mode_id}")
            _validate_mode_types(mode, defaults["modes"].get(mode_id, defaults["modes"]["standard"]), field=f"modes.{mode_id}")
    migrated = _migrate_run_modes_config_v60p(payload)
    # Normalize user aliases before merging packaged CPU defaults. Otherwise
    # an explicit hailo10 GPU selection would conflict with a default hailo10h.
    for supplied in (migrated.get("modes") or {}).values():
        supplied_hailo = (supplied.get("build") or {}).get("hailo") or {}
        if "compute_by_family" in supplied_hailo:
            supplied_hailo["compute_by_family"] = normalize_compute_by_family(supplied_hailo["compute_by_family"])
    data = _deep_merge(defaults, migrated)
    for supplied_id, supplied_mode in (payload.get("modes") or {}).items():
        supplied_hailo = (supplied_mode.get("build") or {}).get("hailo") or {}
        if supplied_id in _MODE_ORDER:
            # An old registry never made an explicit family decision. Keep
            # that absence so its named legacy env remains visible to the
            # resolver; do not turn newly supplied defaults into user intent.
            # Retain explicit empty/partial maps on repeated validation too.
            data["modes"][supplied_id]["build"]["hailo"]["compute_by_family"] = normalize_compute_by_family(supplied_hailo.get("compute_by_family"))
    final_mode = data["modes"]["final"]
    if final_mode.get("description") in _LEGACY_FINAL_DESCRIPTIONS:
        # Description-only repair, including already-schema-13 registries.
        # No force, integrity or other user-selected policy is inferred here.
        final_mode["description"] = defaults["modes"]["final"]["description"]
    ...
    for mode_id in _MODE_ORDER:
        ...
        h["compute_by_family"] = normalize_compute_by_family(h.get("compute_by_family"))
...
def ensure_run_modes_file(path: str | Path | None = None) -> Path:
    ...
    if not dst.exists():
        try:
            save_run_modes_config(dst, default_run_modes_config(), expected_revision="")
        except RunModesConflictError:
            # Another cooperating process completed initial creation first.
            pass
    return dst


def _read_run_modes_config(src: Path) -> Dict[str, Any]:
    try:
        raw = yaml.safe_load(src.read_text(encoding="utf-8"))
    except Exception as exc:
        raise RuntimeError(f"Could not read run-mode configuration {src}: {exc}") from exc
    return validate_run_modes_config(raw)


def load_run_modes_config(path: str | Path | None = None) -> Dict[str, Any]:
    """Read and normalize without rewriting an existing registry for a preview.

    Explicit saves persist the normalized schema. Read-only summaries must not
    become competing configuration writers or rewrite historical evidence.
    """
    return _read_run_modes_config(ensure_run_modes_file(path))


def run_modes_revision(config: Mapping[str, Any]) -> str:
    """Existing normalized configuration identity, also used for save CAS."""
    return _json_hash(validate_run_modes_config(config))


def _changed_run_mode_fields(before: Mapping[str, Any], after: Mapping[str, Any], prefix: str = "") -> list[str]:
    fields: list[str] = []
    for key in sorted(set(before) | set(after)):
        location = f"{prefix}.{key}" if prefix else str(key)
        left, right = before.get(key), after.get(key)
        if isinstance(left, Mapping) and isinstance(right, Mapping):
            fields.extend(_changed_run_mode_fields(left, right, location))
        elif key not in before or key not in after or left != right:
            fields.append(location)
    return fields


class RunModesConflictError(RuntimeError):
    def __init__(self, path: Path, *, expected_revision: str, current_revision: str, changed_fields: list[str]):
        self.path = path
        self.expected_revision = expected_revision
        self.current_revision = current_revision
        self.changed_fields = changed_fields
        detail = ", ".join(changed_fields[:8]) or "Registryrevision"
        super().__init__(f"run_modes_write_conflict: {path}; geändert: {detail}. Neu laden, Änderungen prüfen und erneut speichern.")


@contextmanager
def _run_modes_write_lock(path: Path):
    """Small cooperative configuration lock, independent of hardware locks.

    The stable lock inode must not be removed after use. Non-cooperating
    external editors are outside this compare-and-replace contract.
    """
    with path.with_name(f".{path.name}.lock").open("a+b") as handle:
        if os.name == "posix":
            ...
            raise RuntimeError("run_modes_write_lock_unsupported")
        try:
            yield
        finally:
            if os.name == "posix":
...
def save_run_modes_config(
    path: str | Path | None, payload: Mapping[str, Any], *,
    expected_revision: str | None = None,
    baseline: Mapping[str, Any] | None = None,
) -> Path:
    """Compare and atomically save under the cooperative registry lock.

    Editors must pass the revision captured when loading. An empty revision
    means create only if absent. Legacy direct replacement callers capture the
    current revision at entry; productive read/modify/write callers pass their
    loaded revision explicitly.
    """
    dst = Path(path).expanduser().resolve() if path else default_run_modes_path().resolve()
    dst.parent.mkdir(parents=True, exist_ok=True)
    if expected_revision is None:
        expected_revision = run_modes_revision(_read_run_modes_config(dst)) if dst.exists() else ""
    with _run_modes_write_lock(dst):
        current = _read_run_modes_config(dst) if dst.exists() else None
        current_revision = run_modes_revision(current) if current is not None else ""
        if current_revision != expected_revision:
            changed = _changed_run_mode_fields(baseline or {}, current or {}) if baseline is not None else []
            raise RunModesConflictError(dst, expected_revision=expected_revision, current_revision=current_revision, changed_fields=changed)
        data = validate_run_modes_config(payload)
        descriptor, tmp_name = tempfile.mkstemp(prefix=f".{dst.name}.", suffix=".tmp", dir=dst.parent)
        tmp = Path(tmp_name)
        try:
            with os.fdopen(descriptor, "w", encoding="utf-8", newline="\n") as handle:
                if dst.exists():
                    os.fchmod(handle.fileno(), stat.S_IMODE(dst.stat().st_mode))
...
def reset_run_mode(mode_id: str, path: str | Path | None = None) -> Path:
    ...
    revision = run_modes_revision(data)
    data["modes"][mid] = copy.deepcopy(default_run_modes_config()["modes"][mid])
    return save_run_modes_config(path, data, expected_revision=revision, baseline=baseline)
...
def get_run_mode(mode_id: str, config: Mapping[str, Any] | None = None) -> Dict[str, Any]:
    cfg = validate_run_modes_config(config if config is not None else load_run_modes_config())
...
def mode_summary(mode_id: str, config: Mapping[str, Any] | None = None) -> str:
    ...
    deepx = ((mode.get("build") or {}).get("deepx") or {}) if isinstance(mode.get("build"), Mapping) else {}
    ...
    return (
        ...
        f"Buildpolitik: {_build_policy_text(hailo, deepx)}\n"
        f"Hailo Force: {_force_text(hailo.get('force_build', False), field=f'modes.{mode_id}.build.hailo.force_build')} · "
        f"DeepX Force: {_force_text(deepx.get('force_build', False), field=f'modes.{mode_id}.build.deepx.force_build')}\n"
        f"Hailo-Integrität: {hailo.get('cache_integrity', 'relaxed')} · "
        f"DeepX Classification: {deepx.get('classification_preprocessing', 'current_scale_only')} (Modusdefault)\n"
        + hailo_compute_summary_text({"hailo_build": hailo})
    )
...
def _materialized_blocks(mode_id: str, mode: Mapping[str, Any], *, profile_name: str) -> Dict[str, Any]:
    _validate_mode_types(mode, default_run_modes_config()["modes"][mode_id], field=f"modes.{mode_id}")
    ...
    deepx_build_block: Dict[str, Any] = {
        ...
        "force_build": parse_config_bool(deepx_cfg.get("force_build", False), field=f"modes.{mode_id}.build.deepx.force_build"),
    }
    ...
    for key in ("compiler_overlay", "compiler_venv", "dx_all_suite_root", "diagnostic_only"):
        if key in deepx_cfg:
            deepx_build_block[key] = copy.deepcopy(deepx_cfg[key])
    ...
    blocks: Dict[str, Any] = {
        ...
        "hailo_build": {
            ...
            "compute_by_family": normalize_compute_by_family(hailo_cfg.get("compute_by_family")),
            "hw_arch": "hailo8",
            "targets": ["hailo8"],
            ...
            "force_build": parse_config_bool(hailo_cfg.get("force_build", False), field=f"modes.{mode_id}.build.hailo.force_build"),
...
def apply_run_mode(
    ...
    *,
    ...
    validate_profile_config_booleans(original)
    ...
    follow = parse_config_bool(preset.get("follow_tool_config", True) if follow_tool_config is None else follow_tool_config, field="execution_preset.follow_tool_config")
    ...
    if not follow:
        ...
        cfg = validate_run_modes_config(config if config is not None else load_run_modes_config(cfg_path))
    ...
    if "stop_after" in old_workflow:
        ...
        if stop_after is not None and not isinstance(stop_after, str):
            raise ValueError("config_stage_invalid:workflow.stop_after")
        # An explicitly bounded preparation copy keeps the existing normal
        # stage-completion boundary after reload; it is not a new workflow.
        resolved.setdefault("workflow", {})["stop_after"] = stop_after
    ...
    old_build_provenance = preset.get("build_provenance") if isinstance(preset.get("build_provenance"), Mapping) else {}
    compute_was_default = (
        old_build_provenance.get("hailo_compute_source") in {"tool_config", "profile_snapshot"}
        and old_build_provenance.get("hailo_compute_by_family") == old_hailo_build.get("compute_by_family")
    )
    if "compute_by_family" in old_hailo_build and not compute_was_default:
        # Device choice is a local compiler axis, like the target family.
        # Preserve it without preserving stale build effort from old modes.
        resolved["hailo_build"]["compute_by_family"] = normalize_compute_by_family(
            old_hailo_build["compute_by_family"]
        )
    classification_was_default = (
        old_build_provenance.get("deepx_classification_source") in {"tool_config", "profile_snapshot"}
        and old_build_provenance.get("deepx_classification_preprocessing") == old_deepx_build.get("classification_preprocessing")
    )
    # Local compiler selection and explicit diagnostic intent belong to the
    # profile; preserving them does not preserve calibration/build effort.
    for key in ("compiler_overlay", "compiler_venv", "dx_all_suite_root", "diagnostic_only"):
        if key in old_deepx_build:
            resolved.setdefault("deepx_build", {})[key] = copy.deepcopy(old_deepx_build[key])
    if "classification_preprocessing" in old_deepx_build:
        ...
        if not classification_was_default:
    ...
    classification_source = "tool_config" if follow else "profile_snapshot"
    if "classification_preprocessing" in old_deepx_build and not classification_was_default:
        classification_source = "evaluation_profile"
    resolved["execution_preset"]["build_provenance"] = {
        "deepx_classification_preprocessing": resolved["deepx_build"].get("classification_preprocessing", "current_scale_only"),
        "deepx_classification_source": classification_source,
        "hailo_compute_by_family": copy.deepcopy(resolved["hailo_build"].get("compute_by_family", {})),
        "hailo_compute_source": (
            "evaluation_profile" if "compute_by_family" in old_hailo_build and not compute_was_default
            else "tool_config" if follow else "profile_snapshot"
        ),
    }
    ...
    audit["build_summary"] = profile_build_summary(resolved)
    return resolved, audit


def _force_text(value: Any, *, field: str) -> str:
    return "AN – kompatible Cachetreffer werden bewusst übergangen" if parse_config_bool(value, field=field) else "AUS"


def _build_policy_text(hailo: Mapping[str, Any], deepx: Mapping[str, Any]) -> str:
    modes = [str(block.get("mode") or "reuse_and_build_missing") for block in (hailo, deepx)]
    if modes == ["reuse_and_build_missing", "reuse_and_build_missing"]:
        return "vorhandene passende Artefakte wiederverwenden, fehlende bauen"
    return f"Hailo={modes[0]}, DeepX={modes[1]}"


def profile_build_summary(profile: Mapping[str, Any]) -> Dict[str, Any]:
    """Report effective build values and their resolved source, without I/O."""
    validate_profile_config_booleans(profile)
    preset = profile.get("execution_preset") or {}
    hailo = profile.get("hailo_build") or {}
    deepx = profile.get("deepx_build") or {}
    mid = str(preset.get("id") or infer_run_mode(profile))
    follow = parse_config_bool(preset.get("follow_tool_config", True), field="execution_preset.follow_tool_config")
    if not preset:
        source = "Evaluationsprofil (kein zentraler Override)"
    elif not follow:
        source = f"gebundener Profilsnapshot / {mid}"
    else:
        source = f"Tool Config / {mid} / {preset.get('config_path') or default_run_modes_path()}"
    provenance = preset.get("build_provenance") if isinstance(preset.get("build_provenance"), Mapping) else {}
    classification_source = provenance.get("deepx_classification_source")
    if classification_source == "evaluation_profile" or (classification_source is None and "classification_preprocessing" in deepx):
        classification_source_text = "Evaluationsprofil"
    elif classification_source == "profile_snapshot":
        classification_source_text = f"gebundener Profilsnapshot / {mid}"
    else:
        classification_source_text = source
    from .build_dispatch_policy import active_native_force_fields
    compute = profile_hailo_compute_summary(profile)
    native_force = active_native_force_fields(profile)
    return {
        "build_policy": _build_policy_text(hailo, deepx),
        ...
        "hailo_force_text": _force_text(hailo.get("force_build", False), field="hailo_build.force_build"),
        "deepx_force_text": _force_text(deepx.get("force_build", False), field="deepx_build.force_build"),
        "values_source": source,
        "hailo_cache_integrity": str(hailo.get("cache_integrity") or "relaxed"),
        "deepx_classification_preprocessing": str(deepx.get("classification_preprocessing") or "current_scale_only"),
        "deepx_classification_source": classification_source_text,
        "hailo_compute_by_family": compute,
        "hailo_compute_text": hailo_compute_summary_text(profile),
        "native_force_fields": native_force,
        "native_force_text": "AN: " + ", ".join(native_force) if native_force else "AUS",
        "hailo_cache_enabled": parse_config_bool(hailo.get("cache_enabled", True), field="hailo_build.cache_enabled"),
        "artifact_store_enabled": parse_config_bool((profile.get("artifact_store") or {}).get("enabled", True), field="artifact_store.enabled"),
    }


def profile_hailo_compute_summary(profile: Mapping[str, Any]) -> Dict[str, Any]:
    """Describe exactly the family mapping carried to the build dispatcher.

    No GPU probe or framework import occurs. An environmental conflict is
    visible here but only blocks a required cold build, never a valid HIT.
    """
    mapping = normalize_compute_by_family((profile.get("hailo_build") or {}).get("compute_by_family"))
    preset = profile.get("execution_preset") or {}
    provenance = preset.get("build_provenance") or {}
    setting_source = provenance.get("hailo_compute_source", "evaluation_profile" if mapping else "tool_default")
    summary: Dict[str, Any] = {}
    for family in ("hailo8", "hailo10h"):
        try:
            selection = resolve_compute_selection(family, compute_by_family=mapping or None)
            summary[family] = dict(selection, setting_source=setting_source, status="selected")
        except ValueError as exc:
            summary[family] = {
                "family": family, "device": (mapping.get(family) or {}).get("device", "unresolved"),
                "setting_source": setting_source, "status": "conflict",
                "error": str(exc), "cold_build_allowed": False,
                "compatible_cache_reuse_allowed": True,
            }
    return summary


def hailo_compute_summary_text(profile: Mapping[str, Any]) -> str:
    rows = profile_hailo_compute_summary(profile)
    return "\n".join(
        f"{'Hailo8' if family == 'hailo8' else 'Hailo10H'} Compute für neue Builds: "
        f"{row['device'].upper()} (Quelle: {row.get('source', row['setting_source'])}; "
        f"{row['setting_source']})"
        + (f"; Zusatzbestand: {row.get('dependency_manifest') or ('bewusst keiner' if row.get('dependency_manifest') == '' else 'nicht gesetzt')} "
           f"(Quelle: {row.get('dependency_manifest_source', 'unset')}"
           + ("; für CPU nicht verwendet" if row.get('device') == 'cpu' else "") + ")"
           if family == "hailo8" else "")
        + (f" — Konflikt vor Kaltbuild: {row['error']}" if row["status"] == "conflict" else "")
...
def run_mode_profile_brief(profile: Mapping[str, Any]) -> str:
    ...
    build = profile_build_summary(profile)
    return (
        f"{label} · native={'on' if overrides.get('native_enabled') else 'off'} · native full={full_backends if full_cfg.get('enabled') else 'off'} · native energy={'on' if overrides.get('energy_enabled') else 'off'} · generic energy={'on' if generic_energy else 'off'}\n"
        f"Buildpolitik: {build['build_policy']}\nHailo Force: {build['hailo_force_text']} · DeepX Force: {build['deepx_force_text']}\n"
        f"Wertequelle: {build['values_source']}\nHailo-Integrität: {build['hailo_cache_integrity']} · "
        f"DeepX Classification: {build['deepx_classification_preprocessing']} (Quelle: {build['deepx_classification_source']})\n"
        f"{build['hailo_compute_text']}\nNative Force: {build['native_force_text']} · "
        f"Hailo-Cache: {'AN' if build['hailo_cache_enabled'] else 'AUS'} · "
        f"Artifact Store: {'AN' if build['artifact_store_enabled'] else 'AUS'}"
    )
