"""Hardware-independent release-contract smoke test for version 2.68."""
...
def _performance_repetition_defaults_are_exact() -> bool:
    ...
    # A release smoke must remain read-only and independent of a user's
    # ~/.onnx_splitpoint_tool registry.  Validate the packaged defaults
    # directly instead of materialising the per-user run-modes file.
    config = default_run_modes_config()
    for mode, repetitions in expected.items():
        resolved, _audit = apply_run_mode(
            {"name": "v268_smoke", "execution_preset": {"id": mode}},
            mode_id=mode,
            config=config,
        )
...
def main() -> int:
    ...
    result = {
        "schema": "onnx-splitpoint/v268-smoke",
        "schema_version": 1,
