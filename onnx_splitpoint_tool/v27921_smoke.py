"""Hardware-independent release smoke for the v2.79.21 cache closure.

Behavioral publication, duplicate-selection and barrier regressions are run by
...
def main() -> int:
    ...
    checks = {
        "version": __version__ == __release__ == VERSION == "2.79.21",
        "lineage": __development_lineage__ == LINEAGE == "v2.79",
        "build": __build_id__ == WORKFLOW_VERSION == BUILD_ID
        == "v2.79.21-cache-preflight-atomic-publication",
        ...
        "current_aliases": (
            "from .v27921_smoke import" in
            (root / "onnx_splitpoint_tool/v279_smoke.py").read_text(encoding="utf-8")
            and all("run_v27921_small_acceptance.sh" in
