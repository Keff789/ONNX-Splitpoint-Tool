"""Hardware-independent release smoke for the v2.80.2 model-bound CPU references, Native quality blocking and dashboard reporting.
...
NEW_FEATURES = {'v2802_native_quality_blocking', 'v2802_cpu_reference_model_binding', 'v2802_dashboard_model_count_projection'}


from .v2801_smoke import REQUIRED_FEATURES as PREVIOUS_REQUIRED_FEATURES
...
def main() -> int:
    ...
    checks = {
        "version": __version__ == __release__ == VERSION == "2.80.2",
        "lineage": __development_lineage__ == LINEAGE == "v2.79",
        "build": __build_id__ == WORKFLOW_VERSION == BUILD_ID
        == "v2.80.2-model-binding-native-quality-reporting",
        ...
        "current_aliases": (
            "from .v2802_smoke import" in
            (root / "onnx_splitpoint_tool/v279_smoke.py").read_text(encoding="utf-8")
            and all("run_v2802_small_acceptance.sh" in
