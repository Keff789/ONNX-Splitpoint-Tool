"""Hardware-independent release smoke for the v2.80.1 CPU reference dispatch, Native package closure and diagnostic export.
...
NEW_FEATURES = {'v2801_reference_diagnostic_debug_export', 'v2801_management_cpu_reference_dispatch', 'v2801_native_remote_package_barrier', 'v2801_reference_primary_error'}


from .v280_smoke import REQUIRED_FEATURES as PREVIOUS_REQUIRED_FEATURES
...
def main() -> int:
    ...
    checks = {
        "version": __version__ == __release__ == VERSION == "2.80.1",
        "lineage": __development_lineage__ == LINEAGE == "v2.79",
        "build": __build_id__ == WORKFLOW_VERSION == BUILD_ID
        == "v2.80.1-cpu-reference-remote-closure-debugexport",
        ...
        "current_aliases": (
            "from .v2801_smoke import" in
            (root / "onnx_splitpoint_tool/v279_smoke.py").read_text(encoding="utf-8")
            and all("run_v2801_small_acceptance.sh" in
