"""Hardware-independent release smoke for the v2.80.4 required build readiness, explicit Native non-starts and bounded debug export.
...
NEW_FEATURES = {'v2804_saved_hailo8_dependency_context', 'v2804_quality_cancel_accounting', 'v2804_complete_debug_quality_evidence', 'v2804_preserved_recipe_quality_identity'}


from .v2803_smoke import REQUIRED_FEATURES as PREVIOUS_REQUIRED_FEATURES
...
def main() -> int:
    ...
    checks = {
        "version": __version__ == __release__ == VERSION == "2.80.4",
        "lineage": __development_lineage__ == LINEAGE == "v2.79",
        "build": __build_id__ == WORKFLOW_VERSION == BUILD_ID
        == "v2.80.4-hailo8-context-quality-cancel-complete-debug",
        ...
        "current_aliases": (
            "from .v2804_smoke import" in
            (root / "onnx_splitpoint_tool/v279_smoke.py").read_text(encoding="utf-8")
            and all("run_v2804_small_acceptance.sh" in
