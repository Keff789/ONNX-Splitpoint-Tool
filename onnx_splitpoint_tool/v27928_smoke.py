"""Hardware-independent release smoke for the v2.79.28 declared Float32 score roundoff and staged probe.
...
def main() -> int:
    ...
    checks = {
        "version": __version__ == __release__ == VERSION == "2.79.28",
        "lineage": __development_lineage__ == LINEAGE == "v2.79",
        "build": __build_id__ == WORKFLOW_VERSION == BUILD_ID
        == "v2.79.28-decoded-score-roundoff-staged-probe",
        ...
        "current_aliases": (
            "from .v27928_smoke import" in
            (root / "onnx_splitpoint_tool/v279_smoke.py").read_text(encoding="utf-8")
            and all("run_v27928_small_acceptance.sh" in
