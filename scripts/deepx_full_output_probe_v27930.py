#!/usr/bin/env python3
"""Stage the existing Full DXNN locally, then run the v2.79.30 output diagnostic.
...
DEFAULT_RUN = "v27929_acceptance_d_yolo11l_b003_deepx_gpu_20260907_151417"
DEFAULT_BUNDLE = "ONNX-Splitpoint-Tool_v2.79.30_COMPLETE_DELIVERY_BUNDLE"
...
def resolve_metadata(run_dir: Path, model: str = "yolo11l") -> tuple[dict, dict, dict]:
    ...
    request = {
        ...
        "probe_fix": "v27930", "original_run_id": run_dir.name,
...
def resolve_inputs(args: argparse.Namespace) -> tuple[dict, dict, dict[str, Path]]:
    ...
    template = choose_file([payload / "benchmark_suite.py.txt"], role="v27930_probe_template")
    ...
    if len(functions) != 1 or len(functions[0].args.posonlyargs + functions[0].args.args) != 5:
        raise ValueError("v27930_template_probe_api_missing_or_changed")
    here = Path(__file__).resolve().parent
    worker = here / "deepx_full_output_probe_worker_v27930.py"
...
def remote_temp_valid(path: str) -> bool:
    return re.fullmatch(r"/tmp/onnx-v27930-full-probe-[A-Za-z0-9]{10}", path) is not None
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--payload-dir", type=Path, help="Directory containing the ORIGINAL v2.79.30 benchmark_suite.py.txt.")
    ...
    output = Path(tempfile.mkdtemp(prefix=f"deepx_full_probe_v27930_{stamp}_", dir=destination))
    summary = {"diagnostic_only": True, "probe_fix": "v27930", "status": "setup_started",
    ...
    try:
        ...
        print("PROBE_FIX=v27930", flush=True)
        ...
        with tempfile.TemporaryDirectory(prefix=".deepx-probe-transfer-", dir=destination) as temporary:
            ...
            with (output / "transfer_console.log").open("w", encoding="utf-8") as transfer_log:
                made = subprocess.run(ssh_base(remote) + ["mktemp -d /tmp/onnx-v27930-full-probe-XXXXXXXXXX"],
