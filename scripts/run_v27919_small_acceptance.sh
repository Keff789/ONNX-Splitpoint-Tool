#!/usr/bin/env bash
# Hardware-independent acceptance for the focused v2.79.19 maintenance release.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v27919_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27919_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27919_small_acceptance.sh [--report PATH]' \
        '' \
        'Checks calibration warning semantics and EvaluationRun closure without hardware.'
      exit 0
      ;;
    *)
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27919-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27919-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27919-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.19",
    "build_id": "v2.79.19-calibration-warning-evalrun-closure",
    "release_scope": "calibration_warning_and_evalrun_closure",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27919-acceptance-pycache.??????????)
  ...
  printf 'V27919_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27919_smoke
SMOKE=PASS

PYTEST=FAIL
"$PYTHON" -B -m pytest -q -p no:cacheprovider \
  tests/test_v27919_release_closure.py \
  tests/test_v27919_calibration_plausibility_warnings.py \
  tests/test_v27919_additional_evalrun_defects.py \
  ...
  tests/test_accepted_case_required_scope.py \
...
SHELL=FAIL
bash -n scripts/run_v27919_small_acceptance.sh \
  scripts/run_v27918_small_acceptance.sh \
  scripts/run_v279_small_acceptance.sh \
  scripts/run_local_acceptance.sh \
  scripts/update_source_release.sh
SHELL=PASS

echo 'PASS v2.79.19 small acceptance (hardware calibration and real EvaluationRun: NOT_RUN)'
