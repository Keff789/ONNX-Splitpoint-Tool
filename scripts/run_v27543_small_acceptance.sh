#!/usr/bin/env bash
# Fast, local and hardware-free v2.75.43 acceptance. Run; do not source.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: scripts/run_v27543_small_acceptance.sh was sourced.' \
    'Run: bash scripts/run_v27543_small_acceptance.sh' >&2
  return 0
fi
...
printf '1/4 identity, syntax and v2.75.43 compatibility contract\n'
"$PYTHON" -B - <<'PYTHON'
import ast
...
expected = "v2.75.43-legacy-portable-dataset-identity-compatibility"
assert tool.__version__ == "2.75.43", tool.__version__
assert tool.__release__ == "2.75.43", tool.__release__
assert tool.__development_lineage__ == "v2.75.43"
...
for path in (
    ...
    Path("scripts/run_v27542_small_acceptance.sh"),
...
printf '3/4 focused compatibility, canary and retained-path tests\n'
...
"$PYTHON" -B -m onnx_splitpoint_tool.v27543_smoke

printf '%s\n' \
  'PASS v2.75.43 small acceptance' \
