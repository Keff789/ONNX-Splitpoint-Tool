#!/usr/bin/env bash
# Fast, local and hardware-free v2.75.23 acceptance. Run; do not source.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    "SKIP: scripts/run_v27523_small_acceptance.sh was sourced." \
    "Run: bash scripts/run_v27523_small_acceptance.sh" >&2
  return 0
fi
...
expected = "v2.75.23-full-only-canary-launch-repair"
assert tool.__version__ == "2.75.23", tool.__version__
assert tool.__release__ == "2.75.23", tool.__release__
assert tool.__development_lineage__ == "v2.75.23", tool.__development_lineage__
...
bash -n \
  ...
  scripts/run_v27523_small_acceptance.sh \
  scripts/run_v27523_yolov7_quality_canary.sh \
  scripts/run_v27523_yolov7_pack_replay.sh \
...
printf '3/4 focused v2.75.23 regression contracts\n'
"$PYTHON" -B -m pytest -q -p no:cacheprovider --tb=short \
  ...
  tests/test_v27523_canary_launch.py \
...
printf '%s\n' \
  "PASS v2.75.23 small acceptance" \
