#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.80.1 CPU reference dispatch, Native package closure and diagnostic export.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2801_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V2801_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v2801_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v2801-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v2801-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v2801-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.80.1",
    "build_id": "v2.80.1-cpu-reference-remote-closure-debugexport",
    "release_scope": "cpu_reference_remote_closure_debugexport",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v2801-acceptance-pycache.??????????)
  ...
  printf 'V2801_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V2801_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v2801_smoke
SMOKE=PASS
...
"$PYTHON" -B -m pytest -q -ra -o xfail_strict=true -p no:cacheprovider --junitxml="$PYTEST_JUNIT_MAIN" \
  ...
  tests/test_v2801_release_closure.py \
...
SHELL=FAIL
for script in \
  scripts/run_v2801_small_acceptance.sh \
  scripts/run_v2801_short_tests.sh \
  scripts/run_hailo_gpu_compute_v2801.sh \
  scripts/hailo_gpu_diagnostics/run_hailo_gpu_smoke.sh \
  scripts/run_classification_input_probe_v2801.sh \
  scripts/run_hailo10_yolo26_boundary_probe_v2801.sh \
...
echo 'PASS v2.80.1 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
