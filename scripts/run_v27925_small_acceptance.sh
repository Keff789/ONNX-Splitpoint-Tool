#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.79.25 DeepX GPU and quality follow-up.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27925_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27925_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27925_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27925-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27925-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27925-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.25",
    "build_id": "v2.79.25-deepx-gpu-quality-followup",
    "release_scope": "deepx_gpu_quality_followup",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27925-acceptance-pycache.??????????)
  ...
  printf 'V27925_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27925_smoke
SMOKE=PASS

PYTEST=FAIL
"$PYTHON" -B -m pytest -q -ra -p no:cacheprovider \
  tests/test_v27925_release_closure.py \
...
SHELL=FAIL
for script in \
  scripts/run_v27925_small_acceptance.sh \
...
echo 'PASS v2.79.25 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
