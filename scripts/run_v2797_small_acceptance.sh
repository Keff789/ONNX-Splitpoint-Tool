#!/usr/bin/env bash
# Hardware-independent v2.79.7 remaining-changes acceptance gate.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2797_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V2797_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
      (( $# >= 2 )) || {
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v2797_small_acceptance.sh [--report PATH]' \
        '' \
...
if [[ -n "$REPORT_REQUEST" ]]; then
  ...
  REPORT="${TMPDIR:-/tmp}/onnx-splitpoint-v2797-small-acceptance-$$.json"
fi
...
PHASE_SELECTION[release_smoke]='python -m onnx_splitpoint_tool.v2797_smoke'
PHASE_LABEL[R1]='R1 release, aliases, documentation, and historical identity'
PHASE_SELECTION[R1]='test_v2797 release/documentation/current alias; historical v2796 and v2795 identity/documentation'
...
PHASE_SELECTION[product_integration]='inherited product path plus v2797 runtime-generation contracts'
...
PHASE_SELECTION[artifact]='inherited closure plus v2797 current-path/index regressions'
PHASE_LABEL[gate]='YOLO11 Full terminal-admission gate'
PHASE_SELECTION[gate]='v2797 backend-bound YOLO11 verifier, recovery, gate launcher, and long launcher'
...
payload = {
    "schema": "onnx-splitpoint/v2797-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.7",
...
finalize() {
  ...
  if [[ "$report_rc" -ne 0 ]]; then
    printf 'ERROR: v2.79.7 acceptance report write failed (rc=%s).\n' \
      "$report_rc" >&2
    if [[ "$rc" -eq 0 ]]; then
      rc="$report_rc"
    fi
  else
    printf 'V2797_ACCEPTANCE_REPORT=%s\n' "$REPORT"
  fi

  if [[ "$rc" -eq 0 ]]; then
    printf '%s\n' \
      'PASS v2.79.7 small acceptance' \
      'SKIP accelerator compiler and runtime hardware in this local gate'
  else
    printf 'FAIL v2.79.7 small acceptance (rc=%s, phase=%s)\n' \
...
record_phase release_smoke \
  "$PYTHON" -B -m onnx_splitpoint_tool.v2797_smoke
...
record_phase R2 pytest_phase \
  ...
  tests/test_v2797_yolov7_claim_gate_32.py
...
record_phase product_integration pytest_phase \
  ...
  tests/test_v2797_runtime_generation_contracts.py
...
record_phase gate pytest_phase \
  tests/test_v2797_yolo11_backend_bound_gate.py \
  tests/test_v2797_yolo11_recovery_manifest.py \
  tests/test_v2797_yolo11_gate_launcher.py \
  tests/test_v2797_seven_model_overnight_launcher.py
