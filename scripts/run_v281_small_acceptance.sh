#!/usr/bin/env bash
# Hardware-independent acceptance for v2.81 cold compiler readiness, shared Native input, Hailo10 layout and terminal status.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v281_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V281_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v281_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v281-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v281-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v281-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.81",
    "build_id": "v2.81-hailo-cold-preflight-shared-input-layout-terminal-status",
    "release_scope": "hailo_cold_preflight_shared_input_layout_terminal_status",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v281-acceptance-pycache.??????????)
  ...
  printf 'V281_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V281_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v281_smoke
SMOKE=PASS
...
SHELL=FAIL
for script in \
  scripts/run_v281_small_acceptance.sh \
  scripts/run_v281_short_tests.sh \
...
echo 'PASS v2.81 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
