#!/usr/bin/env bash
# Hardware-independent acceptance for the focused v2.79.14 repair.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v27914_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27914_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27914_small_acceptance.sh [--report PATH]' \
        '' \
        'Checks the direct Full-System M.2 calibration without hardware.'
      exit 0
      ;;
    *)
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27914-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27914-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27914-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.14",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27914-acceptance-pycache.??????????)
  ...
  printf 'V27914_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27914_smoke
SMOKE=PASS
...
echo 'PASS v2.79.14 small acceptance'
