#!/usr/bin/env bash
# Verify the retained backend-bound YOLO11 R8B result and start v2.79.16.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27916_seven_model_long_overnight.sh was sourced.' \
...
LONGRUN_PYCACHE_ROOT="$(
  /usr/bin/mktemp -d /tmp/onnx-v27916-longrun-pycache.XXXXXXXXXX
)" || exit 70
...
cleanup_longrun_pycache() {
  ...
  case "${LONGRUN_PYCACHE_ROOT:-}" in
    /tmp/onnx-v27916-longrun-pycache.??????????)
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  bash scripts/run_v27916_seven_model_long_overnight.sh [options]' \
    '' \
    'Options:' \
...
PROFILE_EXPECTED_SHA256=945ff27288f5ace56ebb42a1c9cae2c717156f44672debb5cb8e0d8c8d12847e
PREFLIGHT_ONLY=0
...
PROFILE="$TOOL/profiles/complete_set_7models_v27916_b500_audit20.yaml"
...
YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v27916_yolov7_claim_gate_32.py"
SOURCE_MANIFEST="$TOOL/SOURCE_MANIFEST.json"
ACCEPTANCE="$TOOL/scripts/run_v27916_small_acceptance.sh"
WORKFLOW_LAUNCHER="$TOOL/scripts/run_fresh_standard_workflow.sh"
STATUS_HELPER="$TOOL/scripts/launcher_status_v27916.py"
for required in \
  "$GATE_VERIFIER" \
  "$GATE_PROFILE" \
  "$RECOVERY_HELPER" \
...
[[ "$PROFILE_OBSERVED_SHA256" == "$PROFILE_EXPECTED_SHA256" ]] ||
  die 65 "v2.79.16-Langprofil stimmt nicht mit der Release-Pin überein"
...
expected = "2.79.16"
expected_build = "v2.79.16-guided-full-system-input-calibration"
...
GLOBAL_LOCK="$LOCK_PARENT/v27916_7model_b500_audit20.lock"
...
CONTROL="$OUT_ROOT/v27916_7model_b500_audit20_launch_${STAMP}_$$"
[[ ! -e "$CONTROL" && ! -L "$CONTROL" ]] ||
...
need(profile.get("name") == "complete_set_7models_v27916_b500_audit20", "profile_name")
...
receipt = {
    "schema": "onnx-splitpoint/v27916-seven-model-long-run-admission/v1",
    "schema_version": 1,
    "status": "PASS",
    "ok": True,
    "release_identity": {
        "package_version": "2.79.16",
...
if ! flock -n "$probe_fd"; then
  exec {probe_fd}>&-
  die 75 "Ein anderer v2.79.16-Sieben-Modell-Langlauf hält bereits den Lock"
fi
...
nohup setsid --fork --wait bash -c '
  ...
  umask 077
  worker_pycache="$(
    /usr/bin/mktemp -d /tmp/onnx-v27916-worker-pycache.XXXXXXXXXX
  )" || exit 70
  export PYTHONDONTWRITEBYTECODE=1
  ...
  cleanup_worker_pycache() {
    ...
    case "${worker_pycache:-}" in
      /tmp/onnx-v27916-worker-pycache.??????????)
  ...
  if ! flock -n 9; then
    "$tool/.venv/bin/python" -B "$status_helper" write \
      ...
      --log "$log" --run-root "$run_root" --workflow-rc 75 \
      --detail "global v2.79.16 campaign lock unavailable"
    exit 75
  fi
  exec 8>"$legacy_lock"
  if ! flock -n 8; then
