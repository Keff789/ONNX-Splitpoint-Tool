#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.79.21 cache-preflight and atomic-publication closure.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27921_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27921_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27921_small_acceptance.sh [--report PATH]' \
        '' \
        'Checks final-selection cache planning, publication and artifact reuse without hardware.'
      exit 0
      ;;
    *)
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27921-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27921-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27921-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.21",
    "build_id": "v2.79.21-cache-preflight-atomic-publication",
    "release_scope": "cache_preflight_atomic_publication",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27921-acceptance-pycache.??????????)
  ...
  printf 'V27921_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27921_smoke
SMOKE=PASS

PYTEST=FAIL
"$PYTHON" -B -m pytest -q -ra -p no:cacheprovider \
  tests/test_v27921_release_closure.py \
...
SHELL=FAIL
bash -n scripts/run_v27921_small_acceptance.sh \
...
echo 'PASS v2.79.21 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
