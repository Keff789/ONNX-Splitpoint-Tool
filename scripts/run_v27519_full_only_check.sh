#!/usr/bin/env bash
# Fresh v2.75.19 field run whose acceptance decision is exactly Full-only 18/18.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    "SKIP: scripts/run_v27519_full_only_check.sh was sourced." \
...
if (( $# < 1 || $# > 3 )); then
  printf '%s\n' \
    "Usage: bash scripts/run_v27519_full_only_check.sh PROFILE [MODELS_ROOT [RUNS_ROOT]]" >&2
  exit 64
fi
...
require_write_target(
    ...
    operation="v2.75.19 Full-only RUNS_ROOT",
...
RUN_SNAPSHOT="$(mktemp -p "$RUNS_ROOT" .v27519-full-only-snapshot-XXXXXXXX)"
...
RUN_MARKER="$(mktemp -p "$RUNS_ROOT" .v27519-full-only-marker-XXXXXXXX)"
trap 'rm -f -- "$RUN_MARKER" "$RUN_SNAPSHOT"' EXIT
...
if (( workflow_rc > 1 )); then
  printf 'FAIL Workflow: rc=%d\n' "$workflow_rc" >&2
  exit "$workflow_rc"
fi
...
RUN_DIR="$(
  "$PYTHON" -B - "$RUN_MARKER" "$RUNS_ROOT" "$PROFILE" "$RUN_SNAPSHOT" <<'PYTHON'
import json
import sys
from pathlib import Path
...
for manifest in sorted(root.glob("*/run_manifest.json")):
    ...
    checks = {
        ...
        # postconditions below.  An overall failed workflow is never admissible.
        "terminal_status": manifest_status in {"ok", "partial"},
        "tool_version": str(
        ...
        ) == "v2.75.19-full-only-reference-contract-repair",
        ...
        "summary_status": summary_status == manifest_status,
    }
