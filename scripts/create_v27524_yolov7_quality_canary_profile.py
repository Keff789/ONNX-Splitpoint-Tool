...
The profile deliberately remains a development/acceptance campaign.  It uses
the deep Detection sample and bootstrap axes, but the ordinary Standard Hailo
cache identity.  Hailo compilation is fenced off: both receipt-attested Full
HEFs must already exist before any remote upload can start.
"""
...
def _profile_id() -> str:
    ...
    return f"yolov7_full_only_quality_canary_v27524_{stamp}"
...
def _frozen_acceptance_mode() -> dict[str, Any]:
    ...
    mode["description"] = (
        "Development acceptance with deep Detection and paired-bootstrap "
        "axes plus the Standard Hailo cache identity; no Native, Energy, "
        "ranking or performance claims."
    )
    ...
    mode["build"]["hailo"]["mode"] = "cache_verify_only"
    mode["build"]["hailo"]["full_baseline_cold_build_policy"] = (
        "cache_or_defer"
    )
    mode["build"]["hailo"]["deferred_full_baseline_required"] = True
    mode["quality"].update(
        {
            "profile_id": "task_quality_development_5000_v27524",
...
def verify_profile(profile: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        hailo.get("mode") != "cache_verify_only"
        or hailo.get("preset") != "balanced"
        or int(hailo.get("optimization_level") or -1) != 1
        ...
        or hailo.get("cache_integrity") != "relaxed"
        or int(hailo.get("timeout_s") or 0) != 3600
        or hailo.get("full_baseline_cold_build_policy")
        != "cache_or_defer"
        or hailo.get("deferred_full_baseline_required") is not True
        ...
        raise ValueError(
            "Deep Quality axes and Standard cache-only Hailo axes were not "
            "materialized"
        )
    expected_statistics = {
...
def main() -> int:
    ...
    print(
        "HAILO_AXES="
        f"preset=balanced opt=1 calibration={CALIBRATION_ITEMS} "
        "cache_integrity=relaxed mode=cache_verify_only"
    )
    print("NATIVE=off")
    print("ENERGY=off")
    return 0
