#!/usr/bin/env python3
"""Stage and invoke the v2.75.21 Native Full import closure on all field nodes."""
...
from onnx_splitpoint_tool.workflow.setup_local_trt_dispatch import (  # noqa: E402
...
def resolve_required_remote_setups(
    ...
    if errors:
        raise RuntimeError(
            "v27521_remote_setup_cardinality_invalid:" + repr(errors)
        )
    ...
    for backend in _REQUIRED_BACKENDS:
        ...
        if not ssh:
            raise RuntimeError(f"v27521_remote_host_missing:{backend}")
        if port != 22:
            raise RuntimeError(
                f"v27521_remote_nonstandard_port_not_supported:{backend}:{port}"
            )
        resolved.append({
            "backend": backend,
            "setup_id": setup_id,
...
def run_remote_contract_preflight(
    ...
    *,
    ...
    local_dispatch = validate_setup_local_tensorrt_quality_dispatch(
        profile_payload,
    )
    if local_dispatch.get("ok") is not True:
        raise RuntimeError(
            "v27521_setup_local_tensorrt_dispatch_preflight_failed:"
            + repr(local_dispatch.get("errors"))
        )
    ...
    for setup in resolve_required_remote_setups(profile_payload):
        ...
        for relative, module_name, tokens in native_remote_package_closure():
            ...
            if not expected_sha:
                raise RuntimeError(
                    f"v27521_remote_asset_hash_missing:{setup['backend']}:{relative}"
                )
        ...
        completed = process_runner_fn(
            ...
            label=f"v27521-remote-contract-preflight:{setup['backend']}",
        ...
        if (
            ...
            raise RuntimeError(
                "v27521_remote_contract_preflight_failed:"
    ...
    return {
        "schema": "onnx-splitpoint/v27521-native-remote-contract-preflight",
        "schema_version": 1,
        "ok": len(rows) == 3 and all(row.get("ok") is True for row in rows),
        "setup_local_tensorrt_dispatch": local_dispatch,
