#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.79.29 DeepX output probe and result fixes.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27929_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27929_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27929_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27929-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27929-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27929-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.29",
    "build_id": "v2.79.29-deepx-prepared-full-without-opencv",
    "release_scope": "deepx_output_probe_result_fixes",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27929-acceptance-pycache.??????????)
  ...
  printf 'V27929_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V27929_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27929_smoke
SMOKE=PASS
...
"$PYTHON" -B -m pytest -q -ra -p no:cacheprovider \
  ...
  tests/test_v27929_release_closure.py \
...
SHELL=FAIL
for script in \
  scripts/run_v27929_small_acceptance.sh \
...
echo 'PASS v2.79.29 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
