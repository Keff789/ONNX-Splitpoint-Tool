...
if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v2798_yolo11_r8b_gate.sh was sourced; run it in its own Bash process.' >&2
  return 0
fi
...
PROFILE_SHA256=f66935318d34232e8abb896d6ffbc46e7d671ec52e83827878e3798b83ad18be
MODEL_SHA256=f0fcdf56a4ac24d87ec30c627170492ccad9db80486ec5694df6de65c1b3d147

usage() {
  printf '%s\n' \
    'Usage: run_v2798_yolo11_r8b_gate.sh --recovery-manifest ABS [--out-root ABS] [--models-root ABS]' \
    '' \
    ...
    'read-only, hash-bound v2798 recovery manifest.'
}
...
PROFILE="$TOOL/profiles/yolo11l_v2798_r8b_full_b067_gate.yaml"
WORKFLOW="$TOOL/scripts/run_evaluation_workflow.py"
VERIFIER="$TOOL/scripts/verify_v2798_yolo11_r8b_gate.py"
...
expected_version = "2.79.8"
expected_build = "v2.79.8-yolo11-gate-profile-schema-closure"
observed = {
...
print("V2798_R8B_RELEASE_IDENTITY=PASS")
PY
then
  die 65 "Installierte v2.79.8-Package-/Workflow-Identität stimmt nicht"
fi
...
then
  die 65 "Installierter v2.79.8-Source-Manifest-Vertrag ist inkonsistent"
fi
SOURCE_MANIFEST_SHA256="$(sha256sum "$SOURCE_MANIFEST" | awk '{print $1}')"
printf 'V2798_R8B_SOURCE_MANIFEST=PASS\n'
...
GATE_OUTPUT="$OUTPUT_ROOT/v2798_yolo11_r8b_${STAMP}_$$"
...
RECOVERY_MANIFEST="$GATE_OUTPUT/v2798_recovery_manifest.json"
"$TOOL_PYTHON" -B - \
  "$RECOVERY_MANIFEST_SOURCE" "$RECOVERY_MANIFEST" \
...
if ((verifier_rc == 0)); then
  printf 'V2798_YOLO11_R8B_GATE=PASS\n'
else
  printf 'V2798_YOLO11_R8B_GATE=FAIL\n' >&2
fi
exit "$verifier_rc"
