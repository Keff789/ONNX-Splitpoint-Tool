...
if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2795_seven_model_long_overnight.sh was sourced.' \
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  bash scripts/run_v2795_seven_model_long_overnight.sh [options]' \
    '' \
    'Options:' \
    ...
    '  --profile PATH           Frozen v2.79.2 scientific seven-model profile.' \
...
ACCEPTANCE="$TOOL/scripts/run_v2795_small_acceptance.sh"
WORKFLOW_LAUNCHER="$TOOL/scripts/run_fresh_standard_workflow.sh"
STATUS_HELPER="$TOOL/scripts/launcher_status_v2795.py"
for required in \
  "$GATE_VERIFIER" \
  "$GATE_PROFILE" \
  "$ACCEPTANCE" \
...
if package != expected or distribution != expected:
    raise SystemExit(
        f"v2.79.5 required: package={package!r} distribution={distribution!r}"
    )
print("V2795_RELEASE_IDENTITY=PASS")
PY
...
FROZEN_HARDWARE="$FROZEN_PARENT/v2795_7model_hardware_setups.yaml"
...
if [[ -e "$FROZEN_HARDWARE" ]]; then
  ...
  cmp --silent -- "$HARDWARE_SETUPS" "$FROZEN_HARDWARE" ||
    die 78 "Vorhandener v2.79.5-Hardware-Freeze weicht von der Registry ab"
else
...
CONTROL="$OUT_ROOT/v2795_7model_b500_audit20_launch_${STAMP}_$$"
[[ ! -e "$CONTROL" && ! -L "$CONTROL" ]] ||
...
receipt = {
    "schema": "onnx-splitpoint/v2795-seven-model-long-run-admission/v1",
    "schema_version": 1,
    "status": "PASS",
    "ok": True,
    "profile": {
...
print("V2795_SEVEN_MODEL_LONG_ADMISSION=PASS")
PY
...
if ! flock -n "$probe_fd"; then
  exec {probe_fd}>&-
  die 75 "Ein anderer v2.79.5-Sieben-Modell-Langlauf hält bereits den Lock"
fi
...
nohup setsid --fork --wait bash -c '
  ...
  if ! flock -n 9; then
    "$tool/.venv/bin/python" -B "$status_helper" write \
      ...
      --detail "global v2.79.5 campaign lock unavailable"
    exit 75
  fi
  exec 8>"$legacy_lock"
  if ! flock -n 8; then
