#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.79.31 DeepX output probe and result fixes.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27931_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27931_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27931_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27931-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27931-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27931-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.31",
    "build_id": "v2.79.31-complete-set-quality-integration",
    "release_scope": "complete_set_integration_and_quality_diagnostics",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27931-acceptance-pycache.??????????)
  ...
  printf 'V27931_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V27931_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27931_smoke
SMOKE=PASS
...
"$PYTHON" -B -m pytest -q -ra -p no:cacheprovider --junitxml="$PYTEST_JUNIT_GUI" \
  tests/test_v27912_energy_setup_claim_admission.py::test_gui_energy_matrix_preflight_blocks_primary_workload
"$PYTHON" -B -m pytest -q -ra -p no:cacheprovider --junitxml="$PYTEST_JUNIT_MAIN" \
  -k 'not test_gui_energy_matrix_preflight_blocks_primary_workload' \
  tests/test_v27931_release_closure.py \
...
SHELL=FAIL
for script in \
  scripts/run_v27931_small_acceptance.sh \
...
echo 'PASS v2.79.31 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
