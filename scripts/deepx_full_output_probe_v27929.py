#!/usr/bin/env python3
"""Stage the existing Full DXNN locally, then run the v2.79.29 output diagnostic.
...
DEFAULT_BUNDLE = "ONNX-Splitpoint-Tool_v2.79.29_COMPLETE_DELIVERY_BUNDLE"
...
def resolve_metadata(run_dir: Path, model: str = "yolo11l") -> tuple[dict, dict, dict]:
    ...
    request = {
        ...
        "probe_fix": "v27929", "original_run_id": run_dir.name,
...
def resolve_inputs(args: argparse.Namespace) -> tuple[dict, dict, dict[str, Path]]:
    ...
    template = choose_file([payload / "benchmark_suite.py.txt"], role="v27929_probe_template")
    ...
    if len(functions) != 1 or len(functions[0].args.posonlyargs + functions[0].args.args) != 5:
        raise ValueError("v27929_template_probe_api_missing_or_changed")
    here = Path(__file__).resolve().parent
    worker = here / "deepx_full_output_probe_worker_v27929.py"
...
def remote_temp_valid(path: str) -> bool:
    return re.fullmatch(r"/tmp/onnx-v27929-full-probe-[A-Za-z0-9]{10}", path) is not None
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--payload-dir", type=Path, help="Directory containing the ORIGINAL v2.79.29 benchmark_suite.py.txt.")
    ...
    output = Path(tempfile.mkdtemp(prefix=f"deepx_full_probe_v27929_{stamp}_", dir=destination))
    summary = {"diagnostic_only": True, "probe_fix": "v27929", "status": "setup_started",
    ...
    try:
        ...
        print("PROBE_FIX=v27929", flush=True)
        ...
        with tempfile.TemporaryDirectory(prefix=".deepx-probe-transfer-", dir=destination) as temporary:
            ...
            with (output / "transfer_console.log").open("w", encoding="utf-8") as transfer_log:
                made = subprocess.run(ssh_base(remote) + ["mktemp -d /tmp/onnx-v27929-full-probe-XXXXXXXXXX"],
