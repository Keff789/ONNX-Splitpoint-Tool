#!/usr/bin/env bash
# Hardware-independent v2.79.13 release-closure acceptance gate.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v27913_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi

set -Eeuo pipefail
PYCACHE_ROOT="$(
  /usr/bin/mktemp -d /tmp/onnx-v27913-acceptance-pycache.XXXXXXXXXX
)" || exit 70
...
cleanup_pycache() {
  ...
  case "${PYCACHE_ROOT:-}" in
    /tmp/onnx-v27913-acceptance-pycache.??????????)
...
REPORT_REQUEST="${V27913_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27913_small_acceptance.sh [--report PATH]' \
        '' \
        'Runs the hardware-independent v2.79.13 identity, platform-power,' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27913-small-acceptance-$$.json}"
...
payload = {
    "schema": "onnx-splitpoint/v27913-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.13",
...
finish_acceptance() {
  ...
  write_report "$rc"
  report_rc=$?
  printf "V27913_ACCEPTANCE_REPORT=%s\n" "$REPORT"
  cleanup_pycache || cleanup_rc=$?
...
SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27913_smoke
SMOKE=PASS
...
# while explicitly allowing preserved user profiles and the updater's exact
# real top-level operational directories; archive verification in the updater
# stays strict release scope.
...
echo 'PASS v2.79.13 small acceptance'
exit 0
