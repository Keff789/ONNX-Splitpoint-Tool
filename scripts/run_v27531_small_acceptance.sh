#!/usr/bin/env bash
# Fast, local and hardware-free v2.75.31 acceptance. Run; do not source.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: scripts/run_v27531_small_acceptance.sh was sourced.' \
    'Run: bash scripts/run_v27531_small_acceptance.sh' >&2
  return 0
fi
...
expected = "v2.75.31-compact-debug-ranking-gui-repair"
assert tool.__version__ == "2.75.31", tool.__version__
assert tool.__release__ == "2.75.31", tool.__release__
assert tool.__development_lineage__ == "v2.75.31", tool.__development_lineage__
...
PYTHON
bash -n \
  scripts/run_v27531_small_acceptance.sh \
...
printf '3/4 focused v2.75.31 debug-pack, ranking, profile and release contracts\n'
...
printf '%s\n' \
  'PASS v2.75.31 small acceptance' \
