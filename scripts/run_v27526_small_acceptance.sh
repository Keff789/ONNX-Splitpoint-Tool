#!/usr/bin/env bash
# Fast, local and hardware-free v2.75.26 acceptance. Run; do not source.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    "SKIP: scripts/run_v27526_small_acceptance.sh was sourced." \
    "Run: bash scripts/run_v27526_small_acceptance.sh" >&2
  return 0
fi
...
expected = "v2.75.26-trt-quality-join-mean-iou-repair"
assert tool.__version__ == "2.75.26", tool.__version__
assert tool.__release__ == "2.75.26", tool.__release__
assert tool.__development_lineage__ == "v2.75.26", tool.__development_lineage__
...
bash -n \
  ...
  scripts/run_v27526_small_acceptance.sh \
...
printf '3/4 focused v2.75.26 regression contracts\n'
"$PYTHON" -B -m pytest -q -p no:cacheprovider --tb=short \
  tests/test_v269d_trt_quality_chain.py \
  tests/test_v269d_quality_first_native_final_chain.py \
...
printf '%s\n' \
  "PASS v2.75.26 small acceptance" \
