#!/usr/bin/env bash
# Hardware-free AP50:95/AP50/AP75 replay from a v2.75.23 Debug Pack.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v27523_yolov7_pack_replay.sh was sourced.' >&2
  return 0
fi
set -Eeuo pipefail
...
if (( $# < 1 || $# > 2 )); then
  printf 'Usage: bash scripts/run_v27523_yolov7_pack_replay.sh DEBUG_PACK [OUT_DIR]\n' >&2
  exit 64
fi
if [[ ! -x "$PY" ]]; then
...
OUT="${2:-${PACK%.zip}_offline_replay_v27523}"
mkdir -p -- "$(dirname -- "$OUT")"
...
TMP_REPLAY="$(mktemp -d -p "$OUT_PARENT" .v27523-pack-replay.XXXXXXXX)"
cleanup_tmp() {
  case "$TMP_REPLAY" in
    "$OUT_PARENT"/.v27523-pack-replay.*) rm -rf -- "$TMP_REPLAY" ;;
