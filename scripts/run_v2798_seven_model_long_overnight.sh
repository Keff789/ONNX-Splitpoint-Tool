#!/usr/bin/env bash
# Verify the backend-bound v2.79.8 YOLO11 R8B result and start the frozen run.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2798_seven_model_long_overnight.sh was sourced.' \
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  bash scripts/run_v2798_seven_model_long_overnight.sh [options]' \
    '' \
    'Options:' \
    '  --yolo11-r8b-output PATH Retained v2.79.8 YOLO11 R8B output (required).' \
...
PROFILE_EXPECTED_SHA256=d168d7ab5069a6cd75db43ec59ea5345de02497f6f2a2abb148f9170ff6de5d4
PREFLIGHT_ONLY=0
...
PROFILE="$TOOL/profiles/complete_set_7models_v2798_b500_audit20.yaml"
...
GATE_VERIFIER="$TOOL/scripts/verify_v2798_yolo11_r8b_gate.py"
GATE_PROFILE="$TOOL/profiles/yolo11l_v2798_r8b_full_b067_gate.yaml"
RECOVERY_HELPER="$TOOL/scripts/prepare_v2798_yolo11_r8b_recovery.py"
YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v2798_yolov7_claim_gate_32.py"
SOURCE_MANIFEST="$TOOL/SOURCE_MANIFEST.json"
ACCEPTANCE="$TOOL/scripts/run_v2798_small_acceptance.sh"
WORKFLOW_LAUNCHER="$TOOL/scripts/run_fresh_standard_workflow.sh"
STATUS_HELPER="$TOOL/scripts/launcher_status_v2798.py"
for required in \
  "$GATE_VERIFIER" \
  "$GATE_PROFILE" \
  "$RECOVERY_HELPER" \
...
[[ "$PROFILE_OBSERVED_SHA256" == "$PROFILE_EXPECTED_SHA256" ]] ||
  die 65 "v2.79.8-Langprofil stimmt nicht mit der Release-Pin überein"
...
if package != expected or distribution != expected:
    raise SystemExit(
        f"v2.79.8 required: package={package!r} distribution={distribution!r}"
    )
print("V2798_RELEASE_IDENTITY=PASS")
PY
...
GLOBAL_LOCK="$LOCK_PARENT/v2798_7model_b500_audit20.lock"
...
CONTROL="$OUT_ROOT/v2798_7model_b500_audit20_launch_${STAMP}_$$"
[[ ! -e "$CONTROL" && ! -L "$CONTROL" ]] ||
...
print("V2798_YOLO11_FULL_GATE=PASS")
PY
...
print("V2798_YOLOV7_RETAINED_CLAIM_RECEIPT=PASS")
PY
...
print("V2798_YOLOV7_CLAIM_GATE_32=PASS")
PY
...
need(profile.get("name") == "complete_set_7models_v2798_b500_audit20", "profile_name")
...
need(gate_verdict.get("version") == "2.79.8", "gate_verdict_version")
need(
...
receipt = {
    "schema": "onnx-splitpoint/v2798-seven-model-long-run-admission/v1",
    "schema_version": 1,
    "status": "PASS",
    "ok": True,
    "profile": {
...
print("V2798_SEVEN_MODEL_LONG_ADMISSION=PASS")
PY
...
if ! flock -n "$probe_fd"; then
  exec {probe_fd}>&-
  die 75 "Ein anderer v2.79.8-Sieben-Modell-Langlauf hält bereits den Lock"
fi
...
nohup setsid --fork --wait bash -c '
  ...
  if ! flock -n 9; then
    "$tool/.venv/bin/python" -B "$status_helper" write \
      ...
      --detail "global v2.79.8 campaign lock unavailable"
    exit 75
  fi
  exec 8>"$legacy_lock"
  if ! flock -n 8; then
