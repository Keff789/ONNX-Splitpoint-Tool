#!/usr/bin/env bash
# Verify the retained backend-bound YOLO11 R8B result and start v2.79.14.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27914_seven_model_long_overnight.sh was sourced.' \
...
# Isolate every Python import from pre-existing source-local bytecode caches.
LONGRUN_PYCACHE_ROOT="$(
  /usr/bin/mktemp -d /tmp/onnx-v27914-longrun-pycache.XXXXXXXXXX
)" || exit 70
export PYTHONDONTWRITEBYTECODE=1
export PYTHONPYCACHEPREFIX="$LONGRUN_PYCACHE_ROOT"

cleanup_longrun_pycache() {
  local exit_status=$?
  trap - EXIT
  case "${LONGRUN_PYCACHE_ROOT:-}" in
    /tmp/onnx-v27914-longrun-pycache.??????????)
      if [[ -L "$LONGRUN_PYCACHE_ROOT" ]]; then
        /bin/rm -- "$LONGRUN_PYCACHE_ROOT" || exit_status=70
      elif [[ -d "$LONGRUN_PYCACHE_ROOT" ]]; then
        /bin/rm -r -- "$LONGRUN_PYCACHE_ROOT" || exit_status=70
      elif [[ -e "$LONGRUN_PYCACHE_ROOT" ]]; then
        exit_status=70
      fi
      ;;
    *) exit_status=70 ;;
...
trap cleanup_longrun_pycache EXIT
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  bash scripts/run_v27914_seven_model_long_overnight.sh [options]' \
    '' \
    'Options:' \
...
PROFILE_EXPECTED_SHA256=d5007651a5a4099b880dad563ff9d7d61126e798fef4eb6e0344c7f37e56f2a0
PREFLIGHT_ONLY=0
...
PROFILE="$TOOL/profiles/complete_set_7models_v27914_b500_audit20.yaml"
...
YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v27914_yolov7_claim_gate_32.py"
SOURCE_MANIFEST="$TOOL/SOURCE_MANIFEST.json"
ACCEPTANCE="$TOOL/scripts/run_v27914_small_acceptance.sh"
WORKFLOW_LAUNCHER="$TOOL/scripts/run_fresh_standard_workflow.sh"
STATUS_HELPER="$TOOL/scripts/launcher_status_v27914.py"
for required in \
  "$GATE_VERIFIER" \
  "$GATE_PROFILE" \
  "$RECOVERY_HELPER" \
...
[[ "$PROFILE_OBSERVED_SHA256" == "$PROFILE_EXPECTED_SHA256" ]] ||
  die 65 "v2.79.14-Langprofil stimmt nicht mit der Release-Pin überein"
...
expected = "2.79.14"
expected_build = "v2.79.14-simple-full-system-m2-idle-calibration"
package = str(getattr(onnx_splitpoint_tool, "__version__", ""))
build = str(getattr(onnx_splitpoint_tool, "__build_id__", ""))
distribution = importlib.metadata.version("onnx-splitpoint-tool")
if package != expected or distribution != expected or build != expected_build:
    raise SystemExit(
        "v2.79.14 required: "
        f"package={package!r} distribution={distribution!r} build={build!r}"
    )
print("V27914_RELEASE_IDENTITY=PASS")
PY
...
GLOBAL_LOCK="$LOCK_PARENT/v27914_7model_b500_audit20.lock"
...
CONTROL="$OUT_ROOT/v27914_7model_b500_audit20_launch_${STAMP}_$$"
[[ ! -e "$CONTROL" && ! -L "$CONTROL" ]] ||
...
print("V27914_YOLOV7_CLAIM_GATE_32=PASS")
PY
...
need(profile.get("name") == "complete_set_7models_v27914_b500_audit20", "profile_name")
...
receipt = {
    "schema": "onnx-splitpoint/v27914-seven-model-long-run-admission/v1",
    "schema_version": 1,
    "status": "PASS",
    "ok": True,
    "release_identity": {
        "package_version": "2.79.14",
        "build_id": "v2.79.14-simple-full-system-m2-idle-calibration",
    },
    "profile": {
...
print("V27914_SEVEN_MODEL_LONG_ADMISSION=PASS")
PY
...
if ! flock -n "$probe_fd"; then
  exec {probe_fd}>&-
  die 75 "Ein anderer v2.79.14-Sieben-Modell-Langlauf hält bereits den Lock"
fi
...
if (( PREFLIGHT_ONLY )); then
  printf '%s\n' \
    'V27914_SEVEN_MODEL_LONG_PREFLIGHT=PASS' \
    'V27910_YOLO11_FULL_GATE=PASS' \
    'V27914_YOLOV7_CLAIM_GATE_32=PASS' \
    'LONG_RUN_STARTED=NO' \
...
nohup setsid --fork --wait bash -c '
  ...
  umask 077
  worker_pycache="$(
    /usr/bin/mktemp -d /tmp/onnx-v27914-worker-pycache.XXXXXXXXXX
  )" || exit 70
  export PYTHONDONTWRITEBYTECODE=1
  export PYTHONPYCACHEPREFIX="$worker_pycache"
  cleanup_worker_pycache() {
    local cleanup_rc=0
    case "${worker_pycache:-}" in
      /tmp/onnx-v27914-worker-pycache.??????????)
        if [[ -L "$worker_pycache" ]]; then
          /bin/rm -- "$worker_pycache" || cleanup_rc=70
        elif [[ -d "$worker_pycache" ]]; then
          /bin/rm -r -- "$worker_pycache" || cleanup_rc=70
        elif [[ -e "$worker_pycache" ]]; then
          cleanup_rc=70
        fi
        ;;
      *) cleanup_rc=70 ;;
  ...
  early_worker_exit() {
    ...
    local cleanup_rc=0
    trap - EXIT
    cleanup_worker_pycache || cleanup_rc=$?
    if (( rc == 0 && cleanup_rc != 0 )); then rc="$cleanup_rc"; fi
    exit "$rc"
  }
  trap early_worker_exit EXIT
  status="$1"
  lock="$2"
  tool="$3"
  profile="$4"
  ...
  if ! flock -n 9; then
    "$tool/.venv/bin/python" -B "$status_helper" write \
      ...
      --log "$log" --run-root "$run_root" --workflow-rc 75 \
      --detail "global v2.79.14 campaign lock unavailable"
    exit 75
  fi
  exec 8>"$legacy_lock"
  if ! flock -n 8; then
...
printf '%s\n' \
  ...
  'V27910_YOLO11_FULL_GATE=PASS' \
  'V27914_YOLOV7_CLAIM_GATE_32=PASS' \
  'V27914_SEVEN_MODEL_LONG_RUN=STARTED' \
  'DETACHED=YES' \
  'YOLO11_R8B_WORKFLOW_RERUN=NO' \
  'YOLOV7_CLAIM_GATE_WORKFLOW_RERUN=NO' \
  'YOLO11_ANCHOR_REUSED=b067' \
