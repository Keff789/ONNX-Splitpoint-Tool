#!/usr/bin/env bash
# Hardware-independent v2.79.5 release/launcher/evidence closure gate.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2795_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
printf '1/6 release identity and v2.79.5 contract smoke\n'
"$PYTHON" -B -m onnx_splitpoint_tool.v2795_smoke

printf '2/6 v2.79.5 release, documentation, and product regressions\n'
"$PYTHON" -B -m pytest -q -p no:cacheprovider --tb=short \
  ...
  tests/test_v2794_release_provenance.py \
  tests/test_v2793_release_provenance.py \
...
bash -n \
  ...
  scripts/run_v2794_small_acceptance.sh \
  scripts/run_v2793_small_acceptance.sh \
