#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.79.32 Native Full failure closure and explicit compiler precedence.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27932_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27932_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27932_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27932-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27932-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27932-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.32",
    "build_id": "v2.79.32-native-full-failure-closure",
    "release_scope": "native_full_failure_closure_and_compiler_precedence",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27932-acceptance-pycache.??????????)
  ...
  printf 'V27932_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V27932_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27932_smoke
SMOKE=PASS
...
"$PYTHON" -B -m pytest -q -ra -o xfail_strict=true -p no:cacheprovider --junitxml="$PYTEST_JUNIT_MAIN" \
  ...
  tests/test_v27932_release_closure.py \
...
SHELL=FAIL
for script in \
  scripts/run_v27932_small_acceptance.sh \
  scripts/run_classification_input_probe_v27932.sh \
  scripts/run_hailo10_yolo26_boundary_probe_v27932.sh \
...
echo 'PASS v2.79.32 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
