#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.80.3 required build readiness, explicit Native non-starts and bounded debug export.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2803_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V2803_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v2803_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v2803-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v2803-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v2803-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.80.3",
    "build_id": "v2.80.3-build-readiness-native-not-started-debug-export",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v2803-acceptance-pycache.??????????)
  ...
  printf 'V2803_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V2803_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v2803_smoke
SMOKE=PASS
...
"$PYTHON" -B -m pytest -q -ra -o xfail_strict=true -p no:cacheprovider --junitxml="$PYTEST_JUNIT_MAIN" \
  ...
  tests/test_v2803_release_closure.py \
...
SHELL=FAIL
for script in \
  scripts/run_v2803_small_acceptance.sh \
  scripts/run_v2803_short_tests.sh \
  scripts/run_hailo_gpu_compute_v2803.sh \
  scripts/hailo_gpu_diagnostics/run_hailo_gpu_smoke.sh \
  scripts/run_classification_input_probe_v2803.sh \
  scripts/run_hailo10_yolo26_boundary_probe_v2803.sh \
...
echo 'PASS v2.80.3 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
