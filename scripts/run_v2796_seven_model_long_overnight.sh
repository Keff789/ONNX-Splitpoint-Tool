#!/usr/bin/env bash
# Verify the retained v2.79.6 YOLO11 R8B result and start the frozen seven-model run.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2796_seven_model_long_overnight.sh was sourced.' \
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  bash scripts/run_v2796_seven_model_long_overnight.sh [options]' \
    '' \
    'Options:' \
    '  --yolo11-r8b-output PATH Retained v2.79.6 YOLO11 R8B output (required).' \
    ...
    '  --help                   Show this help.' \
    '' \
    'The launcher reruns neither hardware gate, never resumes an earlier' \
...
PROFILE_EXPECTED_SHA256=9804fc7f99bd79c17b71d152ad5a20cb4a42d7f1488eefbf414ee961f9ab7930
PREFLIGHT_ONLY=0
...
PROFILE="$TOOL/profiles/complete_set_7models_v2796_b500_audit20.yaml"
...
GATE_VERIFIER="$TOOL/scripts/verify_v2796_yolo11_r8b_gate.py"
GATE_PROFILE="$TOOL/profiles/yolo11l_v2796_r8b_full_b067_gate.yaml"
YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v2796_yolov7_claim_gate_32.py"
SOURCE_MANIFEST="$TOOL/SOURCE_MANIFEST.json"
ACCEPTANCE="$TOOL/scripts/run_v2796_small_acceptance.sh"
WORKFLOW_LAUNCHER="$TOOL/scripts/run_fresh_standard_workflow.sh"
STATUS_HELPER="$TOOL/scripts/launcher_status_v2796.py"
for required in \
  "$GATE_VERIFIER" \
  "$GATE_PROFILE" \
...
[[ "$PROFILE_OBSERVED_SHA256" == "$PROFILE_EXPECTED_SHA256" ]] ||
  die 65 "v2.79.6-Langprofil stimmt nicht mit der Release-Pin überein"
...
if package != expected or distribution != expected:
    raise SystemExit(
        f"v2.79.6 required: package={package!r} distribution={distribution!r}"
    )
print("V2796_RELEASE_IDENTITY=PASS")
PY
...
GLOBAL_LOCK="$LOCK_PARENT/v2796_7model_b500_audit20.lock"
...
CONTROL="$OUT_ROOT/v2796_7model_b500_audit20_launch_${STAMP}_$$"
[[ ! -e "$CONTROL" && ! -L "$CONTROL" ]] ||
...
R8B_WORKFLOW_RC="$("$TOOL_PYTHON" -B - "$R8B_SOURCE_RECEIPT" <<'PY'
import json
import os
import stat
import sys
...
required = {
    "schema": "onnx-splitpoint/yolo11-r8b-terminal-gate/v1",
    "version": "2.79.6",
...
for key, row in paths.items():
    if not isinstance(row, dict) or row.get("terminal_status") not in {"success", "blocked"}:
...
print("V2796_YOLO11_FULL_GATE=PASS")
PY
...
print("V2796_YOLOV7_RETAINED_CLAIM_RECEIPT=PASS")
PY
...
print("V2796_YOLOV7_CLAIM_GATE_32=PASS")
PY
...
need(profile.get("name") == "complete_set_7models_v2796_b500_audit20", "profile_name")
...
need(
    ...
    == "onnx-splitpoint/yolo11-r8b-terminal-gate/v1",
    "gate_verdict_schema",
)
need(gate_verdict.get("version") == "2.79.6", "gate_verdict_version")
need(
...
for key in sorted(expected_gate_paths):
    ...
    need(terminal_status in {"success", "blocked"}, f"gate_terminal_status:{key}")
    gate_terminal_summary[key] = {
        "terminal_status": terminal_status,
        "technical_classification": str(row.get("technical_classification") or ""),
    }
need(
    int(gate_verdict.get("success_count") or 0)
    + int(gate_verdict.get("blocked_count") or 0)
    == 6,
    "gate_terminal_count",
)
...
receipt = {
    "schema": "onnx-splitpoint/v2796-seven-model-long-run-admission/v1",
    "schema_version": 1,
    "status": "PASS",
    "ok": True,
    "profile": {
...
if any(
    ...
    or row.get("terminal_status") not in {"success", "blocked"}
...
print("V2796_SEVEN_MODEL_LONG_ADMISSION=PASS")
PY
...
if ! flock -n "$probe_fd"; then
  exec {probe_fd}>&-
  die 75 "Ein anderer v2.79.6-Sieben-Modell-Langlauf hält bereits den Lock"
fi
...
nohup setsid --fork --wait bash -c '
  ...
  if ! flock -n 9; then
    "$tool/.venv/bin/python" -B "$status_helper" write \
      ...
      --detail "global v2.79.6 campaign lock unavailable"
    exit 75
  fi
  exec 8>"$legacy_lock"
  if ! flock -n 8; then
