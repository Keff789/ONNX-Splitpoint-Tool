#!/usr/bin/env python3
"""Fail-closed verifier for the v2.79.8 YOLOv7 ``claim_gate_32``.
...
# v2.79.8.  Every release/evidence identity is pinned, and the current
...
CARRY_FORWARD_ORIGIN: Mapping[str, Any] = {
    ...
    "current_snapshot_member_sha256": {
        "onnx_splitpoint_tool/__init__.py": "3b320ed4756ec128d1cdbbd01bcae354b242c8accae5c7f5980df0c3943c2378",
        "onnx_splitpoint_tool/release_identity.py": "2d45ecea9b930612112595e5d34b8748560cdaf6821be1f1c7d174864f463c46",
        "onnx_splitpoint_tool/native_three_stage.py": "6ab076454a231f5a7939c05489a885dba92785e2711aa0595125a79aca25311f",
        "scripts/native_hailo_trt_fifo_from_benchmarkset.py": "da285602e2fb6373b6ee7a3c5eafd7ececc5d8728cc560ad78bcdd38ae674800",
        "scripts/native_hailo_trt_concurrent_three_stage_from_benchmarkset.py": "3b1bec9c25320f7122f0d9d4ea5bb961c32274140a23238ab30aa899c50a1ebd",
    },
}
...
def _validate_carry_forward_computational_source(
    ...
    BUILD_ID line; that one audited identity-only difference is checked against
...
def _normalize_build_id_literal(payload: bytes, build_id: str, label: str) -> bytes:
    literal = ('BUILD_ID = "%s"' % build_id).encode("utf-8")
    _need(payload.count(literal) == 1, "%s_build_id_literal_cardinality" % label)
    return payload.replace(literal, b'BUILD_ID = "<AUDITED_RELEASE_IDENTITY>"')
            ...
            for info, relative in zip(infos, SNAPSHOT_MEMBERS):
                ...
                if not carry_forward:
                    ...
                    if relative in (
                        ...
                        captured_normalized = _normalize_build_id_literal(
                            captured_bytes,
                            str(source_policy.get("build_id") or ""),
                            "captured_concurrent_helper",
                        )
                        current_normalized = _normalize_build_id_literal(
                        ...
                        # metadata in v2.79.7/v2.79.8.  Both ends of this exact
    ...
    *, result_json: Path, evidence_root: Path, source_root: Path,
    ...
    receipt: Dict[str, Any] = {
        ...
        "verification_identity": {
            ...
            "verifier": "verify_v2798_yolov7_claim_gate_32.py",
        },
        "invocation_identity": {
            "invocation_sha256": claimed_invocation_sha,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        print("V2798_YOLOV7_CLAIM_GATE=PASS")
        return 0
