#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.80.4 saved Hailo8 context, honest quality cancellation and complete debug evidence.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2804_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V2804_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v2804_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v2804-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v2804-acceptance-pycache.XXXXXXXXXX)"
...
required_modules = tuple(sorted(p.stem for p in Path('tests').glob('test_v2804_*.py')))
test_runs = []
...
payload = {
    "schema": "onnx-splitpoint/v2804-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.80.4",
    "build_id": "v2.80.4-hailo8-context-quality-cancel-complete-debug",
    "release_scope": "hailo8_context_quality_cancel_complete_debug",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v2804-acceptance-pycache.??????????)
  ...
  printf 'V2804_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V2804_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v2804_smoke
SMOKE=PASS
...
SHELL=FAIL
for script in \
  scripts/run_v2804_small_acceptance.sh \
  scripts/run_v2804_short_tests.sh \
  scripts/run_hailo_gpu_compute_v2804.sh \
  scripts/hailo_gpu_diagnostics/run_hailo_gpu_smoke.sh \
  scripts/run_classification_input_probe_v2804.sh \
  scripts/run_hailo10_yolo26_boundary_probe_v2804.sh \
...
echo 'PASS v2.80.4 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
