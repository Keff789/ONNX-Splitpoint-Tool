#!/usr/bin/env bash
# Hardware-independent v2.79.3 acceptance and inherited regression gate.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s
' 'SKIP: run_v2793_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
printf '1/6 release identity and v2.79.3 contract smoke
'
"$PYTHON" -B -m onnx_splitpoint_tool.v2793_smoke

printf '2/6 v2.79.3 productized Three-Stage regressions
'
"$PYTHON" -B -m pytest -q -p no:cacheprovider --tb=short   tests/test_v2793_productized_three_stage.py   tests/test_v2793_release_provenance.py
...
'
bash -n   scripts/run_v2793_small_acceptance.sh   scripts/run_v2792_small_acceptance.sh   scripts/run_v2792_seven_model_long_overnight.sh   scripts/run_local_acceptance.sh   scripts/update_source_release.sh
"$PYTHON" -B -m compileall -q onnx_splitpoint_tool scripts tests
printf '%s
'   'PASS v2.79.3 small acceptance'   'SKIP accelerator compiler and runtime hardware in this local gate'
