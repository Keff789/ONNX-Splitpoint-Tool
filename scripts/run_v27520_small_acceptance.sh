#!/usr/bin/env bash
# Fast, local and hardware-free v2.75.20 acceptance. Run; do not source.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    "SKIP: scripts/run_v27520_small_acceptance.sh was sourced." \
    "Run: bash scripts/run_v27520_small_acceptance.sh" >&2
  return 0
fi
...
expected = "v2.75.20-remote-boundary-plan-finalization-repair"
assert tool.__version__ == "2.75.20", tool.__version__
...
bash -n \
  ...
  scripts/run_v27520_cache_canary.sh \
  scripts/run_v27520_small_acceptance.sh \
  scripts/run_v27520_full_only_check.sh
...
printf '%s\n' \
  "PASS v2.75.20 small acceptance" \
