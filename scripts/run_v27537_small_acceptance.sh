#!/usr/bin/env bash
# Fast, local and hardware-free v2.75.37 acceptance. Run; do not source.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: scripts/run_v27537_small_acceptance.sh was sourced.' \
    'Run: bash scripts/run_v27537_small_acceptance.sh' >&2
  return 0
fi
...
printf '1/4 identity, syntax and 2.75.37 repair features\n'
"$PYTHON" -B - <<'PYTHON'
import ast
...
expected = "v2.75.37-deepx-full-canary-hailo-gate-repair"
assert tool.__version__ == "2.75.37", tool.__version__
assert tool.__release__ == "2.75.37", tool.__release__
assert tool.__development_lineage__ == "v2.75.37"
...
assert Path(
    "profiles/resnet50_v27537_deepx_full_quality_canary.yaml"
).is_file()
...
PYTHON
bash -n \
  scripts/run_v27537_small_acceptance.sh \
...
"$PYTHON" -B -m pytest -q -p no:cacheprovider --tb=short \
  ...
  tests/test_v27537_ready_deepx_canary_profile.py \
...
printf '%s\n' \
  'PASS v2.75.37 small acceptance' \
