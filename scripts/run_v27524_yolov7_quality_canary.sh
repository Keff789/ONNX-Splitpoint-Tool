#!/usr/bin/env bash
# One-command v2.75.24 YOLOv7 Full-only Quality acceptance and pack verification.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27524_yolov7_quality_canary.sh was sourced.' \
...
if (( $# > 3 )); then
  printf '%s\n' \
    'Usage: bash scripts/run_v27524_yolov7_quality_canary.sh' \
...
expected_version = "2.75.24"
expected_build = "v2.75.24-standard-hef-preupload-gate"
...
print("PASS v2.75.24 identity")
PY

PY="$PY" bash scripts/run_v27524_small_acceptance.sh

STAMP="$(date +%Y%m%d_%H%M%S)"
PROFILE_ID="yolov7_full_only_quality_canary_v27524_${STAMP}"
PROFILE="$PROFILE_ROOT/${PROFILE_ID}.yaml"
CANARY_LOG="$RUNS_ROOT/${PROFILE_ID}.log"

"$PY" -B scripts/create_v27524_yolov7_quality_canary_profile.py \
  --out "$PROFILE" \
...
printf 'PASS v2.75.24 YOLOv7 Full-only Quality canary and replay-ready pack\n'
