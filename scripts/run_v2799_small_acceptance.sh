#!/usr/bin/env bash
# Hardware-independent v2.79.9 platform-power acceptance gate.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v2799_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V2799_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v2799_small_acceptance.sh [--report PATH]' \
        '' \
        'Runs the hardware-independent v2.79.9 identity, platform-power, registry,' \
        'calibration transaction, CLI, source-manifest and syntax tests.'
      exit 0 ;;
    *) printf 'ERROR: unsupported option: %s\n' "$1" >&2; exit 64 ;;
  esac
done

REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v2799-small-acceptance-$$.json}"
...
payload = {
    "schema": "onnx-splitpoint/v2799-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.9",
...
trap 'rc=$?; write_report "$rc"; printf "V2799_ACCEPTANCE_REPORT=%s\n" "$REPORT"; exit "$rc"' EXIT

"$PYTHON" -B -m onnx_splitpoint_tool.v2799_smoke
SMOKE=PASS
...
RC=0
STATUS=PASS
echo 'PASS v2.79.9 small acceptance'
exit 0
