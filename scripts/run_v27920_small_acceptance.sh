#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.79.20 artifact-reuse closure.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27920_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27920_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27920_small_acceptance.sh [--report PATH]' \
        '' \
        'Checks Hailo, TensorRT and DeepX artifact reuse without hardware.'
      exit 0
      ;;
    *)
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27920-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27920-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27920-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.20",
    "build_id": "v2.79.20-artifact-reuse-closure",
    "release_scope": "artifact_reuse_closure",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27920-acceptance-pycache.??????????)
  ...
  printf 'V27920_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27920_smoke
SMOKE=PASS

PYTEST=FAIL
"$PYTHON" -B -m pytest -q -p no:cacheprovider \
  tests/test_v27920_release_closure.py \
...
SHELL=FAIL
bash -n scripts/run_v27920_small_acceptance.sh \
...
echo 'PASS v2.79.20 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
