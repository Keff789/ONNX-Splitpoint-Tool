...
if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v27910_yolo11_r8b_gate.sh was sourced; run it in its own Bash process.' >&2
  return 0
fi
...
PROFILE_SHA256=f1da0895df2185158a09b60c61caa939f129fa5cf81a5dfca50652e6a3ac1294
MODEL_SHA256=f0fcdf56a4ac24d87ec30c627170492ccad9db80486ec5694df6de65c1b3d147

usage() {
  printf '%s\n' \
    'Usage: run_v27910_yolo11_r8b_gate.sh --recovery-manifest ABS [--out-root ABS] [--models-root ABS]' \
    '' \
    ...
    'read-only, hash-bound v27910 recovery manifest.'
}
...
PROFILE="$TOOL/profiles/yolo11l_v27910_r8b_full_b067_gate.yaml"
WORKFLOW="$TOOL/scripts/run_evaluation_workflow.py"
VERIFIER="$TOOL/scripts/verify_v27910_yolo11_r8b_gate.py"
...
expected_version = "2.79.10"
expected_build = "v2.79.10-urecs-platform-power-release-closure"
observed = {
...
print("V27910_R8B_RELEASE_IDENTITY=PASS")
PY
then
  die 65 "Installierte v2.79.10-Package-/Workflow-Identität stimmt nicht"
fi
...
then
  die 65 "Installierter v2.79.10-Source-Manifest-Vertrag ist inkonsistent"
fi
SOURCE_MANIFEST_SHA256="$(sha256sum "$SOURCE_MANIFEST" | awk '{print $1}')"
printf 'V27910_R8B_SOURCE_MANIFEST=PASS\n'
...
GATE_OUTPUT="$OUTPUT_ROOT/v27910_yolo11_r8b_${STAMP}_$$"
...
RECOVERY_MANIFEST="$GATE_OUTPUT/v27910_recovery_manifest.json"
"$TOOL_PYTHON" -B - \
  "$RECOVERY_MANIFEST_SOURCE" "$RECOVERY_MANIFEST" \
...
if ((verifier_rc == 0)); then
  printf 'V27910_YOLO11_R8B_GATE=PASS\n'
else
  printf 'V27910_YOLO11_R8B_GATE=FAIL\n' >&2
fi
exit "$verifier_rc"
