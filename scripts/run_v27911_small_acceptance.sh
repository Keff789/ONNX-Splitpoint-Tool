#!/usr/bin/env bash
# Hardware-independent v2.79.11 release-closure acceptance gate.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' 'SKIP: run_v27911_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27911_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27911_small_acceptance.sh [--report PATH]' \
        '' \
        'Runs the hardware-independent v2.79.11 identity, platform-power,' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27911-small-acceptance-$$.json}"
...
payload = {
    "schema": "onnx-splitpoint/v27911-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.11",
...
trap 'rc=$?; write_report "$rc"; printf "V27911_ACCEPTANCE_REPORT=%s\n" "$REPORT"; exit "$rc"' EXIT

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27911_smoke
SMOKE=PASS
...
"$PYTHON" -B -m pytest -q \
  ...
  tests/test_v2796_seven_model_overnight_launcher.py::test_v2796_status_helper_is_frozen_and_current_alias_tracks_v27911 \
  tests/test_v2798_seven_model_overnight_launcher.py::test_v2798_status_helper_is_frozen_and_current_alias_tracks_v27911 \
...
echo 'PASS v2.79.11 small acceptance'
exit 0
