#!/usr/bin/env bash
# Hardware-independent v2.79.8 remaining-changes acceptance gate.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v2798_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V2798_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
      (( $# >= 2 )) || {
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v2798_small_acceptance.sh [--report PATH]' \
        '' \
...
if [[ -n "$REPORT_REQUEST" ]]; then
  ...
  REPORT="${TMPDIR:-/tmp}/onnx-splitpoint-v2798-small-acceptance-$$.json"
fi
...
PHASE_SELECTION[release_smoke]='python -m onnx_splitpoint_tool.v2798_smoke'
PHASE_LABEL[R1]='R1 release, aliases, documentation, and historical identity'
PHASE_SELECTION[R1]='test_v2798 release/documentation/current alias; historical v2797, v2796, and v2795 identity/documentation'
...
PHASE_SELECTION[product_integration]='inherited product path plus v2798 runtime-generation contracts'
...
PHASE_SELECTION[R7]='inherited v2796 unlimited-timeout and receipt regressions'
PHASE_LABEL[artifact]='final artifact-index reseal and verification'
PHASE_SELECTION[artifact]='inherited closure plus v2798 current-path/index regressions'
PHASE_LABEL[gate]='YOLO11 Full terminal-admission gate'
PHASE_SELECTION[gate]='v2798 profile schema/real loader, backend-bound verifier, recovery, gate launcher, and long launcher'
...
contracts = {
    ...
    "backend_bound_yolo11_gate": contract_status("gate"),
    "yolo11_gate_profile_schema": contract_status("gate"),
    "yolo11_gate_profile_real_loader_preflight": contract_status("gate"),
    "runtime_generation_identity": contract_status("product_integration"),
    "artifact_index_terminal_v2": contract_status("artifact"),
...
payload = {
    "schema": "onnx-splitpoint/v2798-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.8",
...
finalize() {
  ...
  if [[ "$report_rc" -ne 0 ]]; then
    printf 'ERROR: v2.79.8 acceptance report write failed (rc=%s).\n' \
      "$report_rc" >&2
    if [[ "$rc" -eq 0 ]]; then
      rc="$report_rc"
    fi
  else
    printf 'V2798_ACCEPTANCE_REPORT=%s\n' "$REPORT"
  fi

  if [[ "$rc" -eq 0 ]]; then
    printf '%s\n' \
      'PASS v2.79.8 small acceptance' \
      'SKIP accelerator compiler and runtime hardware in this local gate'
  else
    printf 'FAIL v2.79.8 small acceptance (rc=%s, phase=%s)\n' \
...
record_phase R2 pytest_phase \
  ...
  tests/test_v2798_yolov7_claim_gate_32.py
...
record_phase product_integration pytest_phase \
  ...
  tests/test_v2798_runtime_generation_contracts.py
...
record_phase gate pytest_phase \
  ...
  tests/test_v2798_yolo11_backend_bound_gate.py \
  tests/test_v2798_yolo11_recovery_manifest.py \
  tests/test_v2798_yolo11_gate_launcher.py \
  tests/test_v2798_seven_model_overnight_launcher.py
