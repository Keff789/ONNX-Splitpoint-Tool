#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.80 Hailo reuse, compute environment and runtime cleanup.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v280_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V280_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v280_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v280-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v280-acceptance-pycache.XXXXXXXXXX)"
...
required_modules = ('test_v280_hailo8_compute_env', 'test_v280_runtime_cleanup', 'test_v280_artifact_reuse', 'test_v280_release_scope', 'test_v280_release_closure')
test_runs = []
...
payload = {
    "schema": "onnx-splitpoint/v280-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.80",
    "build_id": "v2.80-hailo-reuse-env-cleanup",
    "release_scope": "hailo_reuse_env_cleanup",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v280-acceptance-pycache.??????????)
  ...
  printf 'V280_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
export V27928_REPLAY_NPZ="${V27928_REPLAY_NPZ:-${V280_REPLAY_NPZ:-}}"

SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v280_smoke
SMOKE=PASS
...
"$PYTHON" -B -m pytest -q -ra -o xfail_strict=true -p no:cacheprovider --junitxml="$PYTEST_JUNIT_MAIN" \
  ...
  tests/test_v280_release_closure.py \
...
SHELL=FAIL
for script in \
  scripts/run_v280_small_acceptance.sh \
  scripts/run_v280_short_tests.sh \
  scripts/run_hailo_gpu_compute_v280.sh \
  scripts/hailo_gpu_diagnostics/run_hailo_gpu_smoke.sh \
  scripts/run_classification_input_probe_v280.sh \
  scripts/run_hailo10_yolo26_boundary_probe_v280.sh \
...
echo 'PASS v2.80 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
