#!/usr/bin/env bash
# Hardware-independent acceptance for the v2.79.24 targeted run repairs.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27924_small_acceptance.sh was sourced; run it with bash.' >&2
  return 0
fi
...
REPORT_REQUEST="${V27924_ACCEPTANCE_REPORT:-}"
while (( $# > 0 )); do
  case "$1" in
    --report)
    ...
    --help|-h)
      printf '%s\n' \
        'Usage: bash scripts/run_v27924_small_acceptance.sh [--report PATH]' \
        '' \
...
REPORT="${REPORT_REQUEST:-${TMPDIR:-/tmp}/onnx-splitpoint-v27924-small-acceptance-$$.json}"
...
PYCACHE_ROOT="$(/usr/bin/mktemp -d /tmp/onnx-v27924-acceptance-pycache.XXXXXXXXXX)"
...
payload = {
    "schema": "onnx-splitpoint/v27924-small-acceptance/v1",
    "schema_version": 1,
    "version": "2.79.24",
    "build_id": "v2.79.24-targeted-run-repairs",
    "release_scope": "targeted_run_repairs",
...
finish() {
  ...
  case "$PYCACHE_ROOT" in
    /tmp/onnx-v27924-acceptance-pycache.??????????)
  ...
  printf 'V27924_ACCEPTANCE_REPORT=%s\n' "$REPORT"
...
SMOKE=FAIL
"$PYTHON" -B -m onnx_splitpoint_tool.v27924_smoke
SMOKE=PASS

PYTEST=FAIL
"$PYTHON" -B -m pytest -q -ra -p no:cacheprovider \
  tests/test_v27924_release_closure.py \
...
echo 'PASS v2.79.24 small acceptance (real cache preflight and hardware EvalRun: NOT_RUN)'
