#!/usr/bin/env bash
# Fresh v2.75.21 field run whose acceptance decision is exactly Full-only 18/18.

v27521_discovery_failure_rc() {
...
v27521_secure_debug_pack_target() {
  local runs_root="${1:?RUNS_ROOT required}"
  local run_name="${2:?run name required}"
  local pack_dir
  pack_dir="$(mktemp -d -p "$runs_root" .v27521-debug-pack.XXXXXXXX)" \
    || return $?
  printf '%s/%s_v27521_debug_pack.zip\n' "$pack_dir" "$run_name"
}

v27521_terminal_wrapper_rc() {
  local workflow_status="${1:?workflow RC required}"
...
if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    "SKIP: scripts/run_v27521_full_only_check.sh was sourced." \
...
if (( $# < 1 || $# > 3 )); then
  printf '%s\n' \
    "Usage: bash scripts/run_v27521_full_only_check.sh PROFILE [MODELS_ROOT [RUNS_ROOT]]" >&2
  exit 64
fi
...
require_write_target(
    ...
    operation="v2.75.21 Full-only RUNS_ROOT",
...
PY="$PYTHON" bash scripts/run_v27521_small_acceptance.sh
...
dispatch = validate_setup_local_tensorrt_quality_dispatch(payload)
if dispatch.get("ok") is not True:
    raise SystemExit(
        "Setup-lokaler TensorRT-Dispatch ist nicht ausführbar: "
        + repr(dispatch.get("errors"))
    )
dispatch_rows = {
    str(row.get("producer") or ""): row
    for row in dispatch.get("setup_dispatches") or []
    if isinstance(row, dict)
}
if set(dispatch_rows) != {"hailo8", "hailo10h", "deepx"}:
    raise SystemExit(
        "Setup-lokaler TensorRT-Dispatch muss exakt drei Producer enthalten: "
        + repr(dispatch_rows)
    )
for producer, row in dispatch_rows.items():
    run_ids = list(row.get("run_ids") or [])
    if run_ids.count("ort_tensorrt") != 1 or not row.get("setup_id"):
        raise SystemExit(
            "TensorRT-Quality-Companion fehlt oder ist doppelt: "
            + repr((producer, row))
        )
    expected_role = (
        "full_performance_owner" if producer == "deepx"
        else "full_quality_only"
    )
    if row.get("tensorrt_execution_role") != expected_role:
        raise SystemExit(
            "TensorRT-Ausführungsrolle ist nicht kanonisch: "
            + repr((producer, row))
        )
if dispatch.get("performance_owner_producer") != "deepx":
    raise SystemExit(
        "DeepX muss einziger TensorRT-Performance-Owner sein: "
        + repr(dispatch)
    )
if dispatch.get("generic_setup_ids") != dispatch.get("native_setup_ids"):
    raise SystemExit(
        "Generic-/Native-Setup-IDs weichen ab: " + repr(dispatch)
    )
if dispatch.get("remote_cpu_reference_run_ids"):
    raise SystemExit(
        "ORT-CPU darf nicht in den Remote-Dispatch gelangen: "
        + repr(dispatch.get("remote_cpu_reference_run_ids"))
    )
print(
    "PASS Profilvertrag: Split=0, Full-Setups=3x2, "
    "setup-lokales TRT=3, Performance-Owner=DeepX"
)
PROFILE_PY

printf '%s\n' \
  "Prüfe die exakte Native-Runtime-Closure auf Hailo-8, Hailo-10H und DeepX."
"$PYTHON" -B scripts/preflight_v27521_native_remotes.py \
  --profile "$PROFILE"

RUN_SNAPSHOT="$(mktemp -p "$RUNS_ROOT" .v27521-full-only-snapshot-XXXXXXXX)"
...
RUN_MARKER="$(mktemp -p "$RUNS_ROOT" .v27521-full-only-marker-XXXXXXXX)"
RUN_STATUS_RESULT="$(
  mktemp -p "$RUNS_ROOT" .v27521-full-only-status-XXXXXXXX
)"
...
if (( discovery_rc != 0 )); then
  failure_rc=1
  v27521_discovery_failure_rc "$workflow_rc" "$discovery_rc" \
    || failure_rc=$?
  exit "$failure_rc"
fi
