#!/usr/bin/env bash
# Verify the retained backend-bound YOLO11 R8B result and start v2.79.17.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27917_seven_model_long_overnight.sh was sourced.' \
...
LONGRUN_PYCACHE_ROOT="$(
  /usr/bin/mktemp -d /tmp/onnx-v27917-longrun-pycache.XXXXXXXXXX
)" || exit 70
...
cleanup_longrun_pycache() {
  ...
  case "${LONGRUN_PYCACHE_ROOT:-}" in
    /tmp/onnx-v27917-longrun-pycache.??????????)
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  bash scripts/run_v27917_seven_model_long_overnight.sh [options]' \
    '' \
    'Options:' \
...
PROFILE_EXPECTED_SHA256=7fd7f76a41bb8411999d9ba6cdbbb047135c95200cb8da4793fdec484fb36c6f
PREFLIGHT_ONLY=0
...
PROFILE="$TOOL/profiles/complete_set_7models_v27917_b500_audit20.yaml"
...
YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v27917_yolov7_claim_gate_32.py"
SOURCE_MANIFEST="$TOOL/SOURCE_MANIFEST.json"
ACCEPTANCE="$TOOL/scripts/run_v27917_small_acceptance.sh"
WORKFLOW_LAUNCHER="$TOOL/scripts/run_fresh_standard_workflow.sh"
STATUS_HELPER="$TOOL/scripts/launcher_status_v27917.py"
for required in \
  "$GATE_VERIFIER" \
  "$GATE_PROFILE" \
  "$RECOVERY_HELPER" \
...
[[ "$PROFILE_OBSERVED_SHA256" == "$PROFILE_EXPECTED_SHA256" ]] ||
  die 65 "v2.79.17-Langprofil stimmt nicht mit der Release-Pin überein"
...
expected = "2.79.17"
expected_build = "v2.79.17-native-energy-row-isolation-closure"
...
print("V27916_RELEASE_IDENTITY=PASS")
PY
...
GLOBAL_LOCK="$LOCK_PARENT/v27917_7model_b500_audit20.lock"
...
CONTROL="$OUT_ROOT/v27917_7model_b500_audit20_launch_${STAMP}_$$"
[[ ! -e "$CONTROL" && ! -L "$CONTROL" ]] ||
...
print("V27916_YOLOV7_CLAIM_GATE_32=PASS")
PY
...
need(profile.get("name") == "complete_set_7models_v27917_b500_audit20", "profile_name")
...
receipt = {
    "schema": "onnx-splitpoint/v27917-seven-model-long-run-admission/v1",
    "schema_version": 1,
    "status": "PASS",
    "ok": True,
    "release_identity": {
        "package_version": "2.79.17",
        "build_id": "v2.79.17-native-energy-row-isolation-closure",
    },
    "profile": {
...
print("V27916_SEVEN_MODEL_LONG_ADMISSION=PASS")
PY
...
if ! flock -n "$probe_fd"; then
  exec {probe_fd}>&-
  die 75 "Ein anderer v2.79.17-Sieben-Modell-Langlauf hält bereits den Lock"
fi
...
if (( PREFLIGHT_ONLY )); then
  printf '%s\n' \
    'V27916_SEVEN_MODEL_LONG_PREFLIGHT=PASS' \
    'V27910_YOLO11_FULL_GATE=PASS' \
    'V27916_YOLOV7_CLAIM_GATE_32=PASS' \
    'LONG_RUN_STARTED=NO' \
...
nohup setsid --fork --wait bash -c '
  ...
  umask 077
  worker_pycache="$(
    /usr/bin/mktemp -d /tmp/onnx-v27917-worker-pycache.XXXXXXXXXX
  )" || exit 70
  export PYTHONDONTWRITEBYTECODE=1
  ...
  cleanup_worker_pycache() {
    ...
    case "${worker_pycache:-}" in
      /tmp/onnx-v27917-worker-pycache.??????????)
  ...
  if ! flock -n 9; then
    "$tool/.venv/bin/python" -B "$status_helper" write \
      ...
      --log "$log" --run-root "$run_root" --workflow-rc 75 \
      --detail "global v2.79.17 campaign lock unavailable"
    exit 75
  fi
  exec 8>"$legacy_lock"
  if ! flock -n 8; then
...
printf '%s\n' \
  ...
  'V27910_YOLO11_FULL_GATE=PASS' \
  'V27916_YOLOV7_CLAIM_GATE_32=PASS' \
  'V27916_SEVEN_MODEL_LONG_RUN=STARTED' \
  'DETACHED=YES' \
  'YOLO11_R8B_WORKFLOW_RERUN=NO' \
  'YOLOV7_CLAIM_GATE_WORKFLOW_RERUN=NO' \
  'YOLO11_ANCHOR_REUSED=b067' \
