#!/usr/bin/env bash
# Verify the backend-bound v2.79.10 YOLO11 R8B result and start the frozen run.

if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
  printf '%s\n' \
    'SKIP: run_v27910_seven_model_long_overnight.sh was sourced.' \
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  bash scripts/run_v27910_seven_model_long_overnight.sh [options]' \
    '' \
    'Options:' \
...
PROFILE_EXPECTED_SHA256=65c926e7da85d1a46ce099e774c20c80e12e3340684dc5d271038957d69022f9
PREFLIGHT_ONLY=0
...
PROFILE="$TOOL/profiles/complete_set_7models_v27910_b500_audit20.yaml"
...
YOLOV7_CLAIM_VERIFIER="$TOOL/scripts/verify_v27910_yolov7_claim_gate_32.py"
SOURCE_MANIFEST="$TOOL/SOURCE_MANIFEST.json"
ACCEPTANCE="$TOOL/scripts/run_v27910_small_acceptance.sh"
WORKFLOW_LAUNCHER="$TOOL/scripts/run_fresh_standard_workflow.sh"
STATUS_HELPER="$TOOL/scripts/launcher_status_v27910.py"
for required in \
  "$GATE_VERIFIER" \
  "$GATE_PROFILE" \
  "$RECOVERY_HELPER" \
...
[[ "$PROFILE_OBSERVED_SHA256" == "$PROFILE_EXPECTED_SHA256" ]] ||
  die 65 "v2.79.10-Langprofil stimmt nicht mit der Release-Pin überein"
...
if package != expected or distribution != expected:
    raise SystemExit(
        f"v2.79.10 required: package={package!r} distribution={distribution!r}"
    )
print("V27910_RELEASE_IDENTITY=PASS")
PY
...
GLOBAL_LOCK="$LOCK_PARENT/v27910_7model_b500_audit20.lock"
...
CONTROL="$OUT_ROOT/v27910_7model_b500_audit20_launch_${STAMP}_$$"
[[ ! -e "$CONTROL" && ! -L "$CONTROL" ]] ||
...
print("V27910_YOLOV7_CLAIM_GATE_32=PASS")
PY
...
need(profile.get("name") == "complete_set_7models_v27910_b500_audit20", "profile_name")
...
receipt = {
    "schema": "onnx-splitpoint/v27910-seven-model-long-run-admission/v1",
    "schema_version": 1,
    "status": "PASS",
    "ok": True,
    "profile": {
...
print("V27910_SEVEN_MODEL_LONG_ADMISSION=PASS")
PY
...
if ! flock -n "$probe_fd"; then
  exec {probe_fd}>&-
  die 75 "Ein anderer v2.79.10-Sieben-Modell-Langlauf hält bereits den Lock"
fi
...
if (( PREFLIGHT_ONLY )); then
  printf '%s\n' \
    'V27910_SEVEN_MODEL_LONG_PREFLIGHT=PASS' \
    'V27910_YOLO11_FULL_GATE=PASS' \
    'V27910_YOLOV7_CLAIM_GATE_32=PASS' \
    'LONG_RUN_STARTED=NO' \
...
nohup setsid --fork --wait bash -c '
  ...
  if ! flock -n 9; then
    "$tool/.venv/bin/python" -B "$status_helper" write \
      ...
      --log "$log" --run-root "$run_root" --workflow-rc 75 \
      --detail "global v2.79.10 campaign lock unavailable"
    exit 75
  fi
  exec 8>"$legacy_lock"
  if ! flock -n 8; then
...
printf '%s\n' \
  ...
  'V27910_YOLO11_FULL_GATE=PASS' \
  'V27910_YOLOV7_CLAIM_GATE_32=PASS' \
  'V27910_SEVEN_MODEL_LONG_RUN=STARTED' \
  'DETACHED=YES' \
  'YOLO11_R8B_WORKFLOW_RERUN=NO' \
  'YOLOV7_CLAIM_GATE_WORKFLOW_RERUN=NO' \
  'YOLO11_ANCHOR_REUSED=b067' \
